...
public sealed class SingleInstanceGuard : IDisposable
{
    ...
    public static SingleInstanceGuard Acquire(string key)
    {
        ...
        return new SingleInstanceGuard(mutex, createdNew, key + ".pipe");
    }
    ...
    {
        _ = Task.Run(() => ListenLoopAsync(onActivateRequested, _cts.Token), CancellationToken.None);
    }
    ...
    private async Task ListenLoopAsync(Action onActivateRequested, CancellationToken ct)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            catch (IOException) { /* istemci yarıda koptu — bir sonraki bağlantıyı beklemeye devam */ }
        }
    }
