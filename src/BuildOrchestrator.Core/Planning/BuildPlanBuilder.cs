...
public sealed class BuildPlanBuilder(WorkspaceScanner scanner, CsprojEvaluator evaluator, EvaluationCache cache)
{
    ...
    /// <summary>
    /// [design v1.14.0 §9] Ana repo DIŞINDAKİ çalışma alanı köklerinden gelen projelerin id kümesi (bkz.
    /// <c>Core/Externals/ExternalWorkspaceResolver</c>). Bu sınıf harici projeleri hiçbir biçimde ÖZEL ELE
    /// ALMAZ — birleşik taramanın içinde sıradan projelerdir, kenarları ve sıraları graftan doğar; buradaki
    /// tek iş düğüme rozeti basmaktır (ikinci bir gezinti açmamak için node kurulurken).
    /// </summary>
    public BuildPlan Build(ScanResult scan, string configuration, IReadOnlyList<LayerPattern>? layerPatterns,
        ...
        BuildCore(scan, configuration, layerPatterns, externalProjectIds);
        ...
        IReadOnlySet<string>? externalProjectIds)
    {
        ...
        for (int i = 0; i < topo.BuildOrder.Count; i++)
        {
            ...
            nodes.Add(new ProjectNode(
                ...
                IsExternal: externalProjectIds?.Contains(id) == true));
        }
