...
> **Paket sürümü: v1.22.0** · tarih: 2026-09-19 — değişenler için → [Sürüm geçmişi](#sürüm-geçmişi)
>
...
>
> **Prototip notu:** `prototype/` klasörü bu pakette YOK — canlı prototip hâlâ v1.19.0'dadır, aşağıdaki
> davranışları temsil eder. Bu sürümde yalnız metin değişti: yeşil artık yalnız aracın kendi derlemesine
> değil, dışarıda (Visual Studio, CI, elle) derlenmiş ve kanıtı kendi girdilerinden yeni olan bir çıktıya
> da verilir; karar etiketi sözlüğü buna göre genişledi (§2.3, §2.4, §5, §8, §9).
...
- Statü glyph'leri = ince halkalı daire içinde çizim: ✓ tik (success), ✗ çarpı (fail), saat (queued), kesikli daire (discovered). Uyarı üçgeni ayrı sabit slottadır — amber, cycle + dep birleşik (v1.7.0). `—` tire artık bir liste/graf statü glyph'i DEĞİL (v1.20.0) — yalnız run-story yüzeylerinde (event stream'deki atlama satırı, konsol başlığındaki koşu sonucu) görünür; satır, node ve sayaç chip'i taşımaz (§2.4, §2.7, §5).
...
### Renk kuralı (v1.20.0 — tek kanal, çıktının durumu)
Node border'ı ve içindeki küp **aynı statü rengini** taşır; ayrı "plan" ya da "cycle" çekirdeği YOK. Görsel durumlar: **bilinmiyor** (kesikli border, gri küp — yalnız Sync yokken) · **güncel** (yeşil — aracın kendisi derlediyse ve girdi içeriği o derlemeyle hâlâ aynıysa, **ya da** hiç bu araçla derlenmemiş olsa bile diskteki derleme kanıtı kendi girdilerinin tümünden yeniyse) · **derlenecek** (gri) · **bozuk** (kırmızı, kanıtlı) · **işaretli** (amber) · **derleniyor** (amber + beads) · **sonuç** yeşil/kırmızı duruma yazılır ve kalır — bir sonraki koşuya kadar sabittir, "son işlemin hikâyesi" değil çıktının o anki durumudur. Aracın kendi derlediği çıktıda kanıt içeriktendir (girdi aynı kaldıkça yeşil kalır); dışarıda derlenmiş bir çıktıda içerik karşılaştırması yoktur — tek kanıt derleme zamanının kendi girdilerinden yeni olmasıdır. **Döngü üyesinde küp her zaman amber** (`vstate` = `cyc` gri border · `cycskip` skipped border) — satırdaki uyarı üçgeninin grafik vekili; çerçeve kendi durumunu (bilinmiyor/derlenecek/derleniyor/sonuç) taşır, küp döngü üyeliğini sürekli işaretler. Grafta ayrı bir uyarı üçgeni yoktur.

### İşlem koreografisi (v1.11.0 · kuyruğu v1.18.0'da değişti — "sıralı teslim dalgası")
Build/Rebuild/Clean/satır aksiyonu/Resolve — hangisi tıklanırsa aynı sıra oynar (§3.1, §9 v1.11.0/4, v1.18.0): nötr an 440ms → **random dalga** (kapsam tek tek amber'a yanar; `markStagger` 110ms/node, toplam ≤1.1s; border + zemin + küp aynı `transition-delay`) → sarı-gri an 300ms → **grilerin vedası** (kapsam dışı 1120ms ease-in-out ile 0.18'e söner) → **teslim dalgası**: kapsam hep birden değil, **yandığı random sırayla** koşu seviyesine (0.13) iner — `settleStagger` = min(40, 700/(n−1)) ms/node gecikme, her node 400ms ease-in-out → **koşu bu kuyrukla örtüşerek başlar**: ilk derlenen node, son node'lar hâlâ sönerken tutuşur.
**Ölü bekleyiş ve koşu başındaki ikinci sönüm YOK (v1.18.0):** eski `wait`/`wait2` fazları (660ms) ve kapsamın 0.45'te bekleyip koşu başında 0.13'e atlaması kaldırıldı — kapsam doğrudan koşu seviyesine iner, faz değişiminde hiçbir opaklık zıplaması olmaz. Adım saatleri (gerçek zaman, W = `markStagger`×(n−1)+380): `wave` 440 · `hold` 440+W · `dimenv` 740+W · `settle` 1300+W · **koşu 2060+W**. Satır listesinde opaklık koreografisi yoktur (v1.13.2); satırlar yalnız dalga rengini taşır.
...
### Building animasyonu — "beads" (ölçüler v1.18.0)
- Derlenen node'un dışında, node ile eş-merkezli yuvarlatılmış-kare yörüngede dolanan sık amber noktalar.
- Çizim: SVG `rect` (fill none, stroke `--amber-text`, **stroke-width 1.6**, linecap round). Nokta deseni `stroke-dasharray: 0.01 (adım−0.01)`; adım = çevre / round(**çevre/3.2**) → desen çevreye TAM bölünür, ek yerinde bindirme olmaz.
- Hareket: `stroke-dashoffset` 0 → −çevre, **2400ms linear infinite** (sabit hız).
- **Yörünge hücreye kelepçeli:** node'dan dışa boşluk `bgap = max(0.8, min(2.8, (pitch − node − 1.6 − 2) / 2))` (pitch = katman bandının hücre adımı, §2.5 yerleşim). Yörünge kutusu = node + 2×`bgap`; SVG kutusu = yörünge + 2×3.2 (stroke payı). Az projede kelepçe kapanmaz (2.8px dışta, eski yer); 200–300 projede node 8–10px'e inerken **mürekkep dahil komşu şeritle arada ≥2px boşluk** kalır — yan yana iki derlenen node'un şeridi değmez, node/küp/seçim border'ı değişmez.
...
- **Satır (36px, alt çizgi `border-subtle`):** soldan sağa:
  ...
  2. **Statü noktası** 8px — şeritle **aynı rengi** taşır (v1.11.0; ayrı plan/cycle kanalı yok), **bilinmiyor modunda** (yalnız Sync yokken) **4 yaylı halka** (v1.12.0, §1.1), işlem başlayınca dolu noktaya çapraz-söner. Tooltip taşımaz. İşaretleme dalgasında şerit ve node ile aynı gecikmeyle yanar.
  ...
  4. Sağ blok (min 134px, sağa yaslı — v1.16.0: en uzun karar etiketi `up to date · just now` mono 10.5px'te 132px sürüyor, 118px'lik eski yuva onu kırpıyordu): hover'da **4 ikon buton (v1.11.0)** — **play** (o projeyi derler — §3.8), **⋯** (satır menüsü: Build · Rebuild · Clean; satıra sağ tık da açar), "Reveal in Explorer" (klasör) ve "Open in Visual Studio" (kod ikonu). İkon butonları **tooltip taşımaz** (yalnız native `title` + `aria-label`); tıklayınca konsola dim not düşer; hover yokken **karar etiketi (v1.20.0)**: mono 10.5px, sağa yaslı, projenin NEDEN derleneceğini (ya da neden atlanacağını) söyleyen sabit sözcükler — `modified` (kendi dosyaları değişti) · `modified · local` (girdilerinden en az biri `git status`'ta kirli) · `affected` (kendi dosyaları aynı, bağımlılığı değişti **ya da** beslediği çıktının havuzdaki kopyası kendi derleme kanıtıyla uyuşmuyor) · `never built` (diskte çıktı yok ya da bu araç o projeyi hiç derlememiş) · `failed · 2h` (bu kaynakla hata verdi — kuyruk = hatanın yaşı) · `up to date · 2h` (güncel, atlanacak; kuyruk = son başarılı derlemenin yaşı — `just now` / `14m` / `2h` / `3d`; **aynı etiket ve aynı kuyruk biçimi dışarıda derlenmiş bir çıktı için de geçerlidir** — `up to date · 5m` Visual Studio'da derlenmiş bir projede de görünür, kaynağı ayırt etmez). Derlenecek satırlar `text-secondary`, güncel satırlar `text-faint`; `·` sonrası kuyruk her zaman `text-faint`. **Statü rengi taşımaz** (kırmızı/yeşil yalnız glyph + şeritte — tek kanal), ikon taşımaz, DS Tooltip taşımaz: gerekçenin uzun hâli ikon butonlarıyla aynı dilde **native `title`** ile verilir (`Its own files changed since the last build`, `Up to date — last built 2h ago`, `Up to date — built outside this tool 5m ago`, `Its own files are newer than its build output`, `Its copy in the shared folder does not match its build output`). Etiket ana proje ve harici proje için AYNI (harici projeler zaten `EXTERNAL PROJECTS` grubunda — v1.14.0; rozet yok). Yuva değişmedi: min 134px, hover'da 4 ikon butonla yer değiştirir, **satırlar arası sıçrama yok** (v1.7.0 kuralı). Karar bilinmiyorsa (Sync yapılmadı ya da branch değişti → `will: unknown`) yuva boş kalır. `failed · retry` KALKTI (v1.20.0) — "yeniden denenecek" sözünü sarı dalga zaten veriyor. Commit çifti (`a3f81c2 → b7e91d4`) KALDIRILDI — gerekçesi §9 v1.16.0'da.
  5. **Statü glyph'i** 14px (halka içinde ✓/✗/saat, başlangıçta kesikli daire — `—` tire artık burada YOK, v1.20.0: bkz. §2.7). **Tooltip YOK** (v1.11.0 — renk + glyph + süre kolonu aynı şeyi söylüyordu). Glyph her zaman GERÇEK statüyü gösterir; uyarılar yandaki slottadır.
  6. **Sabit 14px uyarı slotu**: cycle üyeliği ve/veya depIssue varsa TEK üçgen (12px), **her zaman amber** (v1.11.0 — turuncu/amber ayrımı kalktı). Uyarı **kümülatiftir** (v1.20.0): defterdeki bağımlılık notu durdukça üçgen kalır; bir sonraki koşu onu tek başına silmez, yalnız not düşünce kalkar. Tooltip **tek satır**: `In a dependency cycle` ya da `Dependency issue: Sales.Core +2`; gerekçe ve üye listesi **proje logunda**. Listede tooltip taşıyan tek öğe budur. Satırın kendisi building iken gizlenir. Slot her satırda var — **hiza asla bozulmaz**.
...
- **Katman başlıkları:** 24px, caps 11px + mono sayı (satır adedi); **birikerek yapışır** — i'inci görünür başlık `top = i×24px`'e yapışır, alttakiler kaydıkça üsttekiler asılı kalır. **v1.17.0: başlık bir gezinme kontrolü** — hover'da zemin `surface-raised`, alt çizgi `border`, metin `text-secondary` (80ms), imleç pointer, native `title` = `Jump to <layer>`; tıklama (ve Enter/Space) o grubun **ilk satırını yığılmış başlıkların hemen altına** getirir: hedef `scrollTop = ilk satırın offsetTop − (başlık indeksi + 1) × 24`, smooth (reduced-motion'da anında). Seçim, filtre, konsol ve graf DEĞİŞMEZ — yalnız scroll. Katman katlama (collapse) yok.
...
- **İmleç ve satır hover (v1.17.0):** gövde imleci **standart ok** (`default`) — metin I-beam'i istenmedi; el işareti yalnız tıklanabilir öğelerde. İmlecin altındaki satır bir yüzey adımı açılır: `surface` (#141417) zeminli **tam genişlik bant** (gövde padding'ini aşan −12px kenar boşluğu + 12px iç boşluk), 80ms geçiş. Uzun logda satır takibi kolaylaşır; renk, hiza ve seçilebilirlik değişmez.
...
- **Hover her satırda (v1.17.0):** tıklanabilir (projeli) satır `surface-hover`, tıklanamaz satır bir adım daha sessiz `surface`; imleç tıklanabilirde pointer, diğerinde default — satırın tıklanabilirliği hover'dan okunur. Seçili satırın görünümü değişmez.
...
> **Hover dili — v1.17.0 (bardaki TÜM kontroller için tek kural):**
> - **Nötr kontrol** (Sync · bakım ikonları · sayaç/filtre çipleri · `N behind` · branch/perf): zemin `neutral-700` (#2a2a30), border `neutral-500` (#54545c), **metin VE ikon** `text-primary` — üç kanal birlikte değişir, kontrol türüne göre fark yok. Statü glyph'i (✓ ✗ —) kendi statü rengini korur (renk statü taşır).
> - **Açık/aktif kontrol** (açık filtre çipi, açık popover, yürüyen bakım görevi/Resolve): zemin `amber-soft-hover`, border `amber` — aktif okuması hover`da kaybolmaz.
> - **Segment (`Debug | Release`):** yalnız seçili OLMAYAN seçenek hover alır (`surface-raised` + `text-secondary`); seçili seçenek `surface-overlay`da kalır.
> - Boyut, radius ve renk tonu değişmez (DS kuralı: hover = bir yüzey adımı), geçiş 80ms; disabled kontrol hover almaz.
> - **Build / Stop bu kuralın DIŞINDA:** kendi variant hover`ında kalır — bar`ın tek yüksek sesli kontrolü odur.
>
> Prototip notu: DS bileşenleri hover`ı inline yazdığı için kural `[data-bo-actions] [data-bo-hov="off|on"]` seçicileriyle ve `!important` ile kurulur (`bo-app-css`).

1. `Sync` — secondary sm buton (döngü ikonu). Koşarken, bakım görevi sürerken veya repo yokken disabled.
2. **Bakım grubu (Clean / Optimize / Resolve cycles)** — chip ağırlığında tek kutu: 24px yükseklik, `surface-raised` zemin, 1px `border`, radius-xs, `overflow:hidden`; içinde üç 28×22 ikon buton, aralarında 1px×14 `border` ayraç. **Etiket yok** (bar 1240'ta ancak böyle sığar) — anlam tooltip'te: `Clean — /t:Clean on every solution, then remove bin/, obj/, artifacts/`, `Optimize — restore packages, prune the cache, rebuild the dependency index`, `Resolve cycles — build the 3 cycle projects in two passes: stale references first, then rebuild until they converge` (döngü yokken `— no dependency cycles detected` + disabled). İkonlar Lucide: **eraser** (Clean), **gauge** (Optimize), **unlink** (Resolve — ikon `--status-cycle-text` turuncusu), 12px, currentColor. Yürüyen görevin/Resolve'un butonu `active` olur ve ikon yerine **amber spinner** döner; diğerleri + Sync + Build + branch chip'i o sırada disabled. Davranış → §3.4, §3.7.
3. 1px dikey ayraç.
4. **Sayaç chip'leri — v1.11.0: ÇOKLU filtre** (`filter` bir Set; çipler bağımsız açılır-kapanır, seçili küme **VEYA** ile birleşir — seçili chip'ler **durum filtreleridir**; arama ile VE). Aktif çip **kendi statü renginde** yanar. Set (v1.20.0): `Σ 36` (tümü/filtre temizle) · spinner+`4` (building; boşken gri nokta) · `✓ 14` · `○ 17` · `✗ 5` (chip glyph + sayıdır — 1240px'lik bar sözcüğe yer bırakmaz; sözcükler `Up to date — filter` · `To build — filter` · `Failed — filter` tooltip'te ve ekran okuyucu adında) · `⚠ 3` (döngü ve bekleyen bağımlılık notu birleşik, filtre `warn`, tooltip `In a dependency cycle or waiting on a dependency — filter`). `— (skipped)` chip'i **KALKTI** — "atlanmak" bir durum değil, satır kendi standı rengini (güncel/derlenecek) taşımaya devam eder; koşunun "N skipped" özeti ribbon'da kalır (§2.2). Ayrı `▲` dependency-affected chip'i de kalktı, `⚠`'a birleşti. `⚠` yalnız listede karşılığı varken görünür, boş/gri chip taşımaz. Aktif filtre chip'i vurgulu.
5. Esnek boşluk.
...
6. `branch: main ▾` chip (branch ikonu) → **Branch popover**; seçim ana ağaçta gerçek `git checkout` (v1.21.0 — §2.8, §3.1). Git işlemi yarıdayken (merge/rebase/cherry-pick/revert yarım kaldıysa ya da `index.lock` var, §3.9) chip'te 5px amber nokta + tooltip `Merge in progress — finish or abort it in git` (duruma göre metin); checkout ve pull o sırada kilitli.
6a. **`3 behind` chip (v1.16.0)** — branch chip'inin hemen sağında, **yalnız geride kalınca** görünür: Lucide *arrow-down-to-line* (12px) + mono sayı + `behind`. Nötr chip ağırlığı (amber yok — statü rengi değil). Görünme koşulları: sayı biliniyor (fetch başarılı) **ve** > 0 — tek ağaç olduğu için "başka branch seçili" ayrımı yok, geride olunan her an görünür (v1.21.0: worktree koşulu kalktı). Çevrimdışı (fetch degraded) → sayı bilinmez → chip yok; güncel → chip yok. Koşu/görev sırasında ve **git işlemi yarıdayken** (madde 6) diğer bar kontrolleriyle aynı kilit (disabled). **Tıklama = ff-only pull** (→ §3.9). Tooltip: `Fetch found 3 new commits on origin/main. Click to fast-forward; nothing is merged or rewritten.`
7. `Debug | Release` segment (sm).
8. `perf: Balanced` chip — tıkla döngü: Full(6) → Balanced(4) → Light(2) paralellik. Tooltip YOK (istenmedi).
9. 1px ayraç.
10. **Build split-button** (primary md, play ikonu): sol `Build` (F5, **stale set**) + sağ `▴` ok → yukarı menü **üç madde (v1.11.0)**: `Build` (play · F5) · `Rebuild` (rotate-cw · Ctrl+F5, tümü) · `Clean` (brush — VS *Clean Solution*: yalnız `/t:Clean`, cache'lere dokunmaz, herkes başlangıç moduna döner). Menü ikonları satır menüsüyle aynı ailedendir (play · rotate-cw · brush, tek grid/stroke). Koşarken buton `Stop`'a döner (her koşuda: Build, Rebuild, satırdan build, Resolve). Tooltip yok.

Toast/popup YOK (karar).
...
**Branch (272px):** caps başlık `SWITCH BRANCH` · arama inputu (büyüteç, `Search branches…`) · satırlar (28px): seçilide ✓ amber, değilse branch ikonu; ad mono; aktif branch'te amber `active` rozeti, diğerlerinde mono SHA. Eşleşme yoksa `No branches match "q".` · Alt not: `Switching checks the branch out in your working tree.`
- **Seçim = gerçek `git checkout <branch>`** (uzak branch için izleyen yerel branch oluşur; v1.21.0 — worktree yok, ana ağaçta checkout). Koşu sürerken ve git işlemi yarıdayken kilitli (§3.9). **Kirli ağaç** — Settings → General'daki anahtara göre (§2.9, §3.1): **Stop** (varsayılan) → konsola `N files have uncommitted changes — commit or stash them first`, hiçbir şey yapılmaz, konsol temizlenmez; **Stash and switch** → `git stash push -u -m "build-orchestrator: leaving <branch> for <target>"`, yeni bölüm açılır (konsol temizlenir), ilk satırlar stash ve checkout satırlarıdır, ardından kısa Sync transkripti. Geri uygulama kullanıcının — araç stash'i ne gösterir ne geri uygular.

## 2.9 Settings dialog (880×576) — v1.19.0 "sol raylı iki panel"

> **v1.19.0 bu bölümü baştan yazar.** Tek uzun kolon, bölüm başına 2–4 satırlık paragraf, bölüm başlığına sıkıştırılmış switch ve `Load sample layers` KALKTI. Gerekçe: ayarlar çoğalacak; tek kolon her eklemede daha da sıkışıyordu. Export / Import / Clear davranışı (v1.10.0) ve harici proje kartının yapısı (v1.17.0: grip + path + sil, tür yoldan okunur) DEĞİŞMEDİ.

**Kabuk (üç dialogun ortağı).** `DialogShell`: scrim + `surface-raised` kutu, 1px `border-strong`, radius-lg, tek overlay gölge, 180ms fade+6px giriş, `overflow: hidden`. Gutter **18px** (başlık ve footer), gövde flex — **yükseklik sabit 576px** (`maxHeight: calc(100% - 48px)`), içerik kendi içinde kayar: bölüm değiştirince dialog zıplamaz.

- **Başlık satırı** (`12px 12px 12px 18px`, altta `border-subtle`): `Settings` (15px/600 `text-primary`) + sağda kapat (X) IconButton.
- **Bölüm rayı (196px):** `surface` zemin (dialogdan bir kademe koyu — iki panel okuması buradan gelir), sağda 1px `border-subtle`, padding `10px 8px`, satır arası 2px. Satır: 30px yüksek, `radius-sm`, padding `0 8px`, 13px/500. Pasif `text-dim` + şeffaf zemin · hover `surface-raised` · **aktif `surface-overlay` + `text-primary`** (amber yok — accent statüye ayrılmıştır). Sağda mono 11px sayaç yalnız doluysa: `External projects`, `Layers`. Sıra: **General · Workspace · External projects · Layers**.
- **Sayfa (pane):** `bo-scroll`, padding `20px 20px 24px`. Her sayfa `PaneHead` ile açılır: başlık 15px/600 + **TEK satır** açıklama (12px `text-dim`, `leading-snug`, max 520px). Uzun gerekçe metni yok.

**Standart ayar satırı (`ToggleRow`) — genişleme biçimi bu.** Solda ad (13px/500 `text-primary`) + altında tek satır açıklama (12px `text-dim`, max 430px), sağda kontrol; satırlar arası 1px `border-subtle`, dikey padding 13px. Yeni ayar eklemek = `GENERAL_GROUPS`'a bir satır daha; düzen işi yok. Bağımlı satır (`dep`) üst anahtar kapalıyken %45 opaklık + `pointer-events: none` (layout sabit).

**General**

- `PaneHead`: `How the app starts and behaves. Every setting applies when you save.`
- Grup başlıkları caps 11px `text-dim`: **STARTUP** · **BUILD** · **BRANCHES** (v1.21.0). Gruplar arası 22px.
- STARTUP: `Start with Windows` (*Launch when you sign in to Windows.*) · `Start minimized to tray` (*No window on start — the tray icon brings it back.*, `dep: startWin`) · `Close to tray` (*Closing the window leaves the engine running in the tray.*)
- BUILD: `Pull before build` (*Update every external working copy first — git pull --ff-only or tf get, one command per copy.*)
- **BRANCHES (yeni, v1.21.0):** `Stash and switch branches` (*When the working tree has uncommitted changes, stash them (including untracked files) and switch. Off: switching stops and asks you to commit or stash first.*) — `ToggleRow`, aynı format (ad + tek satır açıklama + sağda switch); varsayılan **kapalı** (§2.8, §3.1, §4).
- NOTIFICATIONS: `Show notifications` (*A tray notification when a build finishes — succeeded or failed.*) — **tek aç/kapa anahtarı**; "yalnız hatada bildir" kuralı İSTENMEDİ (kullanıcı kararı), bildirimler bütün olarak açılır/kapanır. Prototipte görsel karşılığı yok — Windows tarafında tray bildirimi olarak uygulanacak.
- Kalıcılık: `delta-bo-general-v1` = `{startWin, startTray, closeTray, notify, stashOnBranchSwitch}`, varsayılan `{false, false, true, true, false}` (eski `notifyFail` değeri okunur ve `notify`ye taşınır). **Pull kendi anahtarında kalır** (`delta-bo-pull-external-v1`, varsayılan açık) — motor bağlantısı (`eng.pullExternal`) ve davranışı v1.15.0'dan DEĞİŞMEDİ, yalnız yeri değişti.
- Kaydette değişen her anahtar için konsola tek dim satır: `Start with Windows → on`.

**Workspace**

- `PaneHead`: `The folder that holds the solutions. Projects and the dependency graph are discovered from it on Sync.`
- caps `REPOSITORY ROOT` + satır: mono input (placeholder **`D:\src\myapp`** — artık ürüne özel değil) + secondary `Browse…`. Altında 11px `text-faint`: `Required — nothing is discovered without it.`

**External projects**

- `PaneHead`: `Projects outside the repository root — a folder, a solution or a project file; whether the working copy is Git or TFVC is read from the path. They are built before everything else.`
- Kartlar (36px + 6px): grip (sürükle-bırak sıralama) + mono path input (placeholder **`C:\src\shared\MyApp.Common\MyApp.Common.csproj`**) + sil. Kolon başlığı YOK (tek kolon). Boş durum kesikli kutu: `No external projects — only what is discovered under the repository root is built.` Ekleme: ghost `+ Add external project`.
- Altta hairline + tek satır: `Card order sets the order the working copies are updated. Updating them before a build is on/off.` + ghost buton `Pull before build` → rayı **General**'a alır.

**Layers**

- `PaneHead`: `Projects are grouped by the first matching pattern (regex on the project name), top to bottom. Non-matching projects fall under Other.`
- Kolon başlıkları caps `LAYER NAME` (170px) + `PATTERN`. Kart: grip + ad input (170px) + mono regex input (geçersizse invalid) + sil.
- **`+ Add layer` BOŞ satır ekler** (ad ve desen boş) — değer değil **placeholder** gösterilir; satır indeksine göre `LAYER_PLACEHOLDERS` çiftinden: `Core` / `^MyApp\.(Core|Common)\.` · `Infrastructure` / `^MyApp\.(Data|Infrastructure)\.` · `Domain` / `^MyApp\.Domain\.` · `Services` / `^MyApp\.Services\.` · `Api` / `\.Api$` · `Client` / `^MyApp\.(Web|Client|Mobile)\.` (6'dan sonra baştan döner). Ürün adı taşımaz.
- Varsayılan BOŞ; boş durum kesikli kutu: `No layers yet — projects show as a single list in build order.`

**Footer** (`12px 18px`, üstte `border-subtle`)

- Solda **Export / Import / Clear** ikon butonları (davranış v1.10.0): Export → `build-orchestrator-settings.json` (`app`, `version`, `repositoryRoot`, `externalProjects[{path, vcs}]`, `pullExternalBeforeBuild`, **`general{startWithWindows, startMinimizedToTray, closeToTray, notifications}`**, `layers[{name, pattern}]`) · Import → değerleri **forma** yükler (`general` alanı da okunur) · Clear → iki aşamalı ikon onayı (`Click again to clear`), formu boşaltır ve General'ı varsayılanına döndürür. Üçü de Save'e kadar hiçbir şey uygulamaz; geri bildirim aynı satırda 2.4s.
- Geri bildirim yokken ve **Save kapalıyken tek satır neden** (11px `text-faint`): `Repository root is required` · `Every external project needs a path` · `Every layer needs a name` · `Check the highlighted pattern`. Hangi sayfada olunduğu fark etmez.
- Sağda `Cancel` + primary `Save` (first run'da `Save and sync`).
- Açılış bölümü: first run'da **Workspace**, sonrasında **General**.

---

## 2.10 About dialog (620px, F1) — v1.19.0 sadeleştirmesi

> **v1.19.0:** `Third-party` sekmesi KALDIRILDI (placeholder lisans listesiydi). Sekme sırası mantıksal: **About → Environment → Shortcuts**, F1 **About**'ta açar. Sekme kontrolü **`DS.Segment` olarak kaldı** (alt çizgili `DS.Tabs` denendi, kullanıcı segmenti seçti): kontrol bir nesne olduğu için **kutusu** 18px gutter'a hizalanır, etiketin kendi 11px iç boşluğu kontrole aittir. Hizasızlık hissi boşlukla çözüldü — kimlik bloğunun altına 16px, segmentin altına 14px + **tam genişlik hairline**; segment artık başlığa yapışık değil, gövdeyle de kıyaslanmıyor.

Ortak `DialogShell` (§2.9). Yükseklik içeriğe göre; **gövde sabit 284px** — sekme değişince dialog zıplamaz.

- **Kimlik bloğu** (`20px 18px 18px`): `app-mark.svg` (28px) + `Build Orchestrator` (17px/600) + yanında **sürüm çipi** (mono 11px, `surface` zemin, 1px `border-strong`, radius-xs) · alt satır `Ordered, incremental builds for a multi-project .NET solution.` (12px `text-dim`). Sağda firma kilidi: 1px×28px ayraç + caps `LICENSED TO` + firma logosu (13px, %80). Eski `1.18.0 · © 2026 Delta` satırı kalktı — sürüm çipe, copyright About sekmesine taşındı.
- **Sekme bandı:** `DS.Segment` (md, 26px) kendi bandında — band padding `6px 18px 14px`, altta tam genişlik `border-subtle`. **Dikey ritim (kullanıcıyla oturtuldu):** kimlik bloğunun tanım satırından segmente **26px** (kimlik bloğu padding `20px 18px 20px` + band üstü 6px), segmentten hairline'a **14px**, hairline'dan gövde metnine **14px** (gövde padding `14px 18px 20px`) — segmentin iki yanındaki boşluk eşit okunur. Renkler DS varsayılanı: `surface-sunken` kap + 1px `border`, aktif seçenek `surface-overlay` + `text-primary`, pasif `text-dim` — **amber yok** (accent statüde kalır).
- **About sekmesi:** 13px/1.62 paragraf (max 470px): *Build Orchestrator discovers the projects under the repository root, works out the dependency graph and builds in that order — only what changed, in parallel where the graph allows. The plan, the running build and its result stay visible while it works.* · hairline · etiket/değer satırları (etiket 124px 12px `text-dim`, değer mono 12px): `Version` · `Engine` · `Copyright © 2026 Delta Yazılım` · altta ghost buton **`What's new in <sürüm>`** (sparkle ikonu) → About kapanır, What's new açılır.
- **Environment sekmesi:** iki caps grup. **RUNTIME**: Engine PID · .NET runtime · OS. **PATHS**: MSBuild · Repository root · State file · Logs · Worktree pool. Satır 27px, etiket 124px, değer mono 12px; **yollar kesilmez** — hücre `.bo-xscroll` (görünmez yatay scroll, fare tekerleği `scrollLeft`e gider, v1.13.1). App/Engine sürümü bu sekmeden çıktı.
- **Shortcuts sekmesi:** iki caps grup — **BUILD** (F5 · Ctrl+F5/Shift+F5) ve **APPLICATION** (Ctrl+F · F1 · Ctrl+F1 · Esc · Alt+B). Satır 27px: solda açıklama 13px `text-secondary`, sağda Kbd chip'leri. **Kısayol metinleri birebir korunur.**
- **Footer:** solda ghost `Copy diagnostics` (1.4s `Copied` + yeşil ✓; panoya sürüm + engine + Environment satırları düz metin), sağda secondary `Close`. Ghost butonun sarmalayıcısı `margin-left: -10px` ile etiketi 18px gutter'a hizalar.
- Esc önceliği: What's new → About → Settings → popover/menü → seçim. Scrim tıklaması da kapatır.

## 2.11 What's new dialog (720×600, Ctrl+F1)

> **v1.19.0:** genişlik 620 → **720**, gövde 400 → **flex** (dialog 600px sabit, `maxHeight: calc(100% - 48px)`). Sürüm kimliği başlıktan ve satır sonundan **sol sütuna** taşındı; başlıktaki iki satırlı `INSTALLED VERSION` bloğu yerine tek mono sürüm çipi. Kategori blokları (6px kare + caps) ve veri kaynağı (`app/release-notes.js`) DEĞİŞMEDİ.

Title bar'daki **sparkle** ikonundan veya **Ctrl+F1** ile açılır (toggle). Ortak `DialogShell`, zIndex 101 (About 100).

- **Başlık satırı** (`20px 18px 16px`, altta `border-subtle`): solda `What's new` (15px/600) + `Release notes for Build Orchestrator.` (12px `text-dim`); sağda mono sürüm çipi (20px yüksek, `surface` zemin + `border-strong`). Sağ kenar sabit — scrollbar'a göre hizalanan blok yok.
- **Gövde:** `bo-scroll`, padding `22px 18px 24px`, flex (dialog yüksekliği sabit).
- **Sürüm bloğu = 2 kolonlu grid** (`84px` + `1fr`, 26px kolon aralığı):
  - **Sol sütun (sticky, `top: 0`):** mono 13px/500 sürüm · altında mono 11px `text-faint` tarih · kurulu sürümde 16px `INSTALLED` çipi. Uzun bir sürümün maddeleri kayarken kimliği ekranda kalır; tarihler her sürümde aynı sütunda.
  - **Sağ sütun:** kategori blokları, aralarında 15px. Blok: 6px renkli kare + caps `text-dim` başlık, altında 13px maddeler 13px içeriden, **satır arası 10px, satır yüksekliği 1.62, max 500px ölçü** (`text-wrap: pretty`).
- Sürümler arasında 22px boşluk + 1px `border-subtle`.
- Kategoriler ve sırası: **Added** (amber) · **Changed** (`text-dim`) · **Fixed** (yeşil) · **Performance** (turuncu) · **Removed** (kırmızı). Boş kategori çizilmez.
- **Son 3 sürüm açık**, gerisi `Earlier versions (N)` ghost butonu altında; buton grid'in sağ kolonunda, `margin-left: -10px` ile madde kolonuna hizalı.
- **Footer:** yalnız sağda secondary `Close`.
- **Görülmemiş sürüm işareti** (v1.13.0) DEĞİŞMEDİ: `delta-bo-seen-version-v1` ≠ `ABOUT_VERSION` iken sparkle butonunda 5px amber nokta; dialog açıldığı anda düşer. Açılış pop-up'ı YOK.
...
**v1.21.0 — Sync artık kendiliğinden de çalışır (spec §6.2).** İlke: konsol bir **bölüm** anlatır. Yeni bölümü yalnız iki şey açar — **kullanıcının başlattığı bir işlem** (Sync butonu, Build/Rebuild/satırdan/Resolve, Clean/Optimize, Pull) ya da **branch değişimi** (araçtan ya da dışarıdan); ikisi de konsolu temizler ve kendi transkriptiyle baştan yazar. Aynı dünyada kendiliğinden olan tazeleme — **commit** ya da **pencereye dönüş** (son Sync'ten 5 sn geçmediyse tekrarlanmaz) — bölüm açmaz: konsolu/olay akışını temizlemez, listeyi kaydırmaz, reveal oynatmaz, fetch yapmaz (`N behind` yereldeki son bilinen mesafeye göre hesaplanır); olay akışına yalnız tek satır düşer (`synced after commit`, `synced · N projects changed`).

| Tetik | Konsol | Liste / graf | Fetch |
|---|---|---|---|
| Uygulama açılışı · Sync butonu | temizlenir, tam transkript | reveal (açılışta) / yapı aynıysa yerinde (butonda) | evet |
| Branch değişti (araçtan ya da dışarıdan) | temizlenir; ilk satır yeni branch, ardından kısa Sync transkripti | aynı kural | hayır |
| Build / Rebuild / satırdan / Resolve | temizlenir (bugünkü) | renkler kalır | hayır |
| Clean / Optimize | temizlenir; zincirlenen Sync altına eklenir | boşalır, dolar | evet |
| Pull | korunur; sonuç Sync altına eklenir | yerinde | evet |
| Commit (kendiliğinden) | dokunulmaz; tek satır eklenir | yerinde | hayır |
| Pencereye dönüş, 5 sn eşiği (kendiliğinden) | dokunulmaz; değişiklik varsa tek satır eklenir | yerinde | hayır |

**v1.11.0 — kullanıcının başlattığı işlem aynı borudan geçer:** `_beginOp` (konsol + stream temizlenir, kalıcı işlem etiketi) → `_neutralize` (herkes düz nötr gri) → `_mark` (faz `marking`: nötr an → random dalga → sarı-gri an → örtüşen veda → nefes) → koşu. Girişler: `beginBuild` · `beginRebuild` · `beginProject(name, mode)` · `beginProjectClean` · `startTask` · `startResolve`. Koşu bitince `_runEndFinale` (hold → neon → nefes → grilerin açılışı). Ayrıntı: §9 v1.11.0 / 4-5. **Kendiliğinden Sync (commit, pencereye dönüş) bu boruya hiç girmez** (v1.21.0) — koreografi ve reveal yalnız kullanıcı işleminde oynar.
...
- **boot:** repo var, Sync yapılmadı. Konsolda açılış satırı: `Build Orchestrator 2.4.1 — Osys.sln loaded (36 projects) · main`. Tüm satır/node'lar **başlangıç modunda** (satırda soluk düz şerit + 4 yaylı nokta, grafta kesikli node border; v1.12.0). **Açılışta Sync otomatik ve hemen ardından koşar** (v1.21.0) — kullanıcı Sync'e basmaz; boot yalnız bu geçişin anlık görüntüsüdür.
- **Sync:** konsola `▸ git fetch origin main` → **`HEAD a3f81c2 · 3 commits behind origin/main`** (güncelse `HEAD b7e91d4 · up to date with origin/main`; çevrimdışıysa bu satır hiç yazılmaz, yerine degrade uyarısı — v1.16.0) → `Sync complete — 7 changed projects, 14 to build` + `22 projects up to date (will skip)`. **Sync sonrası satırlar durum renginde** (v1.20.0 — §2.3): derlenecek satırlar gri, güncel yeşil, kanıtlı bozuk kırmızı; başlangıç (bilinmiyor) modu artık yalnız hiç Sync yapılmadan önce görülür. Plan aynı zamanda **karar etiketinde okunur** (§2.4): derlenecek satırlar `modified` / `modified · local` / `affected` / `never built`, kanıtlı bozuk `failed · 2h`, kalanlar `up to date · 2h`. Şerit `Ready — 14 changed · 22 up to date`, işlem pill'i `SYNC`. Açılış = otomatik sync, kapat-aç hep bu moda döner. **`.git\logs\HEAD` izlenir** (v1.21.0 — §6.1 spec): checkout, commit, pull, reset görülünce Sync kendiliğinden tetiklenir; HEAD ve branch adı son Sync'tekiyle aynıysa atlanır — aracın kendi checkout'u ve pull'un zincirli Sync'i böylece ikinci bir Sync üretmez.
- **Build (F5):** boot'ta Sync otomatik koşar; diğer her durumda **mevcut durumdan** koşar — stale set = değişen + hatalı + hatalıların bağımlıları (`beginBuild`). **Konsol ve stream her Build'de temizlenir** (v1.11.0) ve koreografi baştan oynar. Bir önceki koşu her şeyi güncellediyse `_reseedChanges()` yeni bir HEAD üretip 2-4 proje bayatlar (kapsam ≤%45) — tekrar Build de tam süreçle koşar. Stale set gerçekten boşsa (Sync sonrası) hızlı kontrol koşusu (`Everything up to date`, koreografi yok). Önceki koşuda hata varsa düzeltme uygulandı varsayılır (baseFails kapanır). Konsola `▸ msbuild Osys.sln /m:4 /p:Configuration=Debug — 14 projects, 22 skipped`. **Git işlemi yarıdayken engellenmez** (v1.21.0 — spec §6.4): merge/rebase/cherry-pick/revert sürerken de Build çalışır; planlama başına işleme göre adlandırılmış tek uyarı satırı eklenir, ör. merge sürerken `a merge is in progress — files with conflict markers will not compile` (rebase/cherry-pick/revert'te işlem adı değişir, dil aynı).
...
- **Stop:** building olanlar queued'a döner; konsola warn `Build stopped — 7/14 completed`, stream'e `Stopped — 7 remaining projects queued`. Sonrası **Build** kaldığı yerden sürdürür; elapsed yeni koşuda sıfırlanır. Koşu sürerken F5 = Stop. **Marking fazında da çalışır** (v1.11.0): koşu hiç başlamaz, konsola `Cancelled — build not started`. **Koşu sırasında branch değişirse** (v1.21.0 — checkout/pull/merge/reset/rebase) nazikçe durur: yeni proje başlamaz, derlenenler biter, değişim anında derlenmekte olanların sonucu deftere yazılmaz; koşu bitince yeni bölüm açılır, ilk satırı koşunun özetidir (kaç proje bitti, kaçı yarıda kaldı, run log klasörünün yolu). Commit koşuyu durdurmaz.
...
- Kilitleme: görev sürerken Sync, Build (+ menü oku), branch chip'i, Debug/Release ve diğer bakım butonu disabled; F5/Ctrl+F5 no-op. Görev durdurulamaz (Stop yok) — 4-5 saniyelik işler.
...
Alt bardaki chip'ler ve şeritteki `+N more` liste filtresini kurar (v1.20.0 — durum filtreleri): `building` (yalnız şu an derlenen) · `current` (up to date — yeşil görünen) · `stale` (to build — gri görünen, kanıtsız hata dahil) · `failed` (kırmızı görünen) · `warn` (döngü ∪ bekleyen bağımlılık notu). Seçili chip'ler VEYA ile birleşir, arama ile VE. `+N more` `failed` durum filtresini açar. Aynı chip'e tekrar tıkla → kümeden çıkar; `Σ` chip'i hepsini temizler. Aktif filtre Projects başlığında kaldırılabilir chip.
...
**Kural (bilinçli güncelleme):** "araç ana repoda pull yapmaz" kuralı duruyor — araç **kendiliğinden asla** pull etmez. Kullanıcı chip'e basarsa **yalnız ff-only**, **yalnız aktif branch'te**. Gerekçe: fetch zaten "3 commit gerideyim" diyor; bunu gördükten sonra terminale geçip `git pull --ff-only` yazmak aracın işini yarıda bırakmak olur. Fast-forward geri alınabilir, tarihi yeniden yazmaz, birleştirme kararı vermez — aracın ana repoda yazdığı üç git işleminden biridir (diğerleri: checkout — §2.8, stash push — §2.8, §3.1).

**Kilit (v1.21.0):** koşu sürerken ve **git işlemi yarıdayken** (merge/rebase/cherry-pick/revert yarım kaldıysa ya da `index.lock` var, §6.4 spec) chip disabled, tooltip nedeni söyler; kirli ağaçta uyarı verir ve pull'u yapmaz (aşağıdaki tablo). Worktree kalktığı için chip'in görünürlüğünde "başka branch seçili" koşulu yok — geride olunan her an görünür (§2.7-6a).
...
- **Pull sonrası Sync konsolu silmez** (v1.21.0 — §3.1'deki genel kendiliğinden-Sync kuralının bir örneği): kullanıcı Sync butonuna basarsa motor resetleyip konsolu temizler, ama pull sonrası zincirlenen Sync geçmişi korur ve altına eklenir — yoksa kullanıcı kendi tetiklediği pull'un sonucunu hiç görmezdi.
...
- **UI durumu:** `selected` (proje adı|null), `filter` (**Set — çoklu, VEYA**; v1.11.0), layout `{mode, col, left, right}` (kalıcı), `layerCfg` (kalıcı), cfg (Debug/Release), perf. **Branch artık ayrı bir UI seçimi değil** (v1.21.0 — worktree kalktı): görüntülenen branch `.git`'ten okunan gerçek aktif branch'tir.
- **Ayar (Settings → General, kalıcı, v1.21.0):** `stashOnBranchSwitch: bool` (varsayılan `false` = *Stop*; `true` = *Stash and switch branches*) — kirli ağaçta branch chip'inden değiştirmenin davranışını seçer (§2.8, §2.9, §3.1).
...
| succeeded | ✓ daire | #58cb80 | Succeeded — kaynağı aracın kendi derlemesi de olabilir (içerik hâlâ aynı), dışarıda (Visual Studio, CI, elle) yapılmış bir derleme de (kanıt zaman olarak kendi girdilerinden yeni); ikisi de aynı yeşili, aynı glyph'i ve aynı `Succeeded` metnini taşır |
| failed | ✗ daire | #ff706a | Failed |
| bozuk (kanıtlı) | ✗ daire | #ff706a | Failed — yalnız derleyici hatası (exit ≠ 0) kanıttır: hata imzası bugünkü imzayla eşit; timeout/Stop kanıt SAYILMAZ, gri `never built` kalır, kırmızıya çevirmez (v1.20.0) |
| ~~skipped~~ (v1.20.0'da ayrı glyph/renk kanalı KALKTI) | — tire artık yalnız run-story yüzeylerinde (event stream'deki atlama satırı, konsol başlığındaki koşu sonucu) | — | satır/node kendi **standı durumunu** taşır (ör. güncelse yeşil ✓); koşunun "N skipped" özeti ribbon'da kalır — run-story ribbon'da yaşamaya devam eder |
| bilinmiyor (`fresh`, eski adı "başlangıç modu") | tam opak düz şerit + 4 yaylı nokta (v1.13.2: ikisi de standart tonda) + kesikli daire glyph (DS, değişmedi) / kesikli node border (değişmedi) | `--status-skipped-border` | tooltip yok — **yalnız Sync yokken** görülür (v1.20.0: Sync sonrası artık durum renginde) |
| cycle üyesi (`cyc` / `cycskip`) | gri node border + **amber küp** (grafta); satırda normal gri + uyarı üçgeni | `--amber-text` | küp döngü üyeliğinde **her zaman** amber (v1.20.0) — çerçeve kendi durumunu taşır, Resolve ile derlense de küp amber kalır |
...
| ~~will-build (ortogonal dot)~~ | **v1.11.0'da KALDIRILDI** — nokta ve node çekirdeği artık statü rengini taşır | — | plan bilgisi **renkte ve etikette** (v1.20.0): derlenecek satır gri, güncel yeşil, kanıtlı bozuk kırmızı; etikette `modified` · `modified · local` · `affected` · `never built` · `failed · 2h` · `up to date · 2h` (eski çift SHA kalktı) |
...
- **v1.20.0:** renk ÇIKTININ DURUMUNU anlatır; tek kanal ilkesi korunur (şerit + nokta + glyph + node border + küp aynı renk). Ayrı will-build noktası ve turuncu döngü işareti GERİ GELMEZ; döngü yalnız amber küp + amber üçgen + log ile ifade edilir. Kırmızı badge eklenmez (kırmızı = kanıtlı bozuk: derleyici hatası, bugünkü imzayla eşleşiyor). Çıktının tarihi tek başına güncel demeye yetmez: araç kendi derlediğinde kanıt içerikten gelir, başkasının derlediğinde kanıt yalnızca zamandan gelir — ikisi karıştırılmaz.
...
- `⚠` chip'i (döngü ve bekleyen bağımlılık notu birleşik; v1.20.0'da eski `▲` dep-affected chip'i buna katıldı) yalnız listede karşılığı varken görünür; kalan beş sayaç chip'i her zaman durur.
...
- **v1.16.0 / v1.21.0:** araç ana repoda **kendiliğinden** git yazmaz — mutasyon yüzeyi yalnız **üç kullanıcı eylemi**: `N behind` chip'ine basınca ff-only pull (yalnız aktif branch'te), branch chip'inden checkout, kirli ağaçta *Stash and switch* ayarıyla stash push (§2.8, §3.1, §3.9). Merge commit'i, rebase, force-push, otomatik pull/checkout/stash YOK. Chip yerine pasif uyarı, şeritte metin ve chip menüsü denendi — İSTENMEDİ (§9 v1.16.0).
...
| Sürüm | Tarih | Konu |
|---|---|---|
| **v1.22.0** | 2026-09-19 | **Dışarıdan derleme kredisi**: yeşil artık yalnız aracın kendi derlemesine değil, kanıtı kendi girdilerinden yeni olan her derlemeye (Visual Studio, CI, elle) verilir · karar etiketi sözlüğünde `never built` ve `affected` genişledi, `up to date · 5m` dışarıda derlenmiş bir çıktıda da aynı biçimde görünür |
| **v1.21.0** | 2026-09-18 | **Tek ağaç ve branch**: worktree kod ve UI'ı kalkar (chip, popover), branch chip'i ana ağaçta gerçek `git checkout` yapar · kirli ağaçta Settings → General anahtarı (*Stop* varsayılan / *Stash and switch*) · Sync artık kendiliğinden de çalışır (`.git\logs\HEAD` izleyici; commit ve pencereye dönüşte sessiz, yalnız kullanıcı işlemi ve branch değişimi konsol bölümü açar) · git işlemi yarıdayken branch chip'inde amber nokta, checkout ve pull kilitli |
| **v1.20.0** | 2026-09-18 | **Renk çıktının durumunu anlatır**: "son işlemin hikâyesi" ilkesi terk edildi — Sync sonrası satırlar durum renginde (yeşil/gri/kırmızı), `bilinmiyor` yalnız hiç Sync yokken görülür · karar etiketi sözlüğü sadeleşti (`modified · local` eklendi, `failed · retry` kalktı) · `—` (skipped) glyph'i ve chip'i kalktı, yalnız run-story yüzeylerinde kalır · döngü küpü her zaman amber · `⚠` chip'i eski `▲` dep-affected chip'ini yuttu |
| v1.19.0 | 2026-09-16 | **Üç dialog yenilendi**: Settings sol raylı iki panel (General bölümü + genişleyebilir ayar satırı, `Load sample layers` kalktı, placeholder'lar standart katman iskeleti) · What's new sürüm kimliği sol sabit sütunda · About sadeleşti (Third-party kalktı, About→Environment→Shortcuts, alt çizgili sekme şeridi) |
| v1.18.0 | 2026-09-14 | **Sıralı teslim dalgası**: kapsam yandığı sırayla koşu seviyesine iner, ölü bekleyiş + koşu başındaki ikinci sönüm kalktı · **beads bir tık kalın** (nokta 1.6px, aralık 3.2px, tur 2400ms) ve yörünge hücreye kelepçeli (komşuyla ≥2px) · **konsol başlığı / `Back` satırı** birebir tarif edildi (değişiklik değil — uygulama tarafı tasarımdan sapmış) |
| v1.17.0 | 2026-09-14 | **Tek hover dili** (alt bar kontrolleri) · katman başlıkları tıklanabilir (gruba scroll) · konsolda metin imleci + satır hover · event stream'de her satırda hover · harici proje kartından `Source` seçimi kalktı |
...
**Dışarıdan derleme kredisi: Visual Studio'da ya da elle derlenen bir proje artık gri kalmıyor — neden: VS'de derlenen proje araçta gri kalıyordu, kullanıcı isteği 2026-09-17 (spec §4, §5, §7).**

- **Renk kuralı genişledi (§2.3).** Yeşil artık iki yoldan gelir: aracın kendi derlediği ve içeriği hâlâ aynı olan çıktı, ya da hiç bu araçla derlenmemiş olsa da diskteki derleme kanıtı kendi girdilerinin tümünden yeni olan çıktı. Aracın derlemesinde kanıt içerikten gelir, dışarıdaki derlemede kanıt yalnızca zamandan gelir.
- **Karar etiketi sözlüğü genişledi (§2.4).** `up to date · 5m` artık Visual Studio'da ya da elle derlenmiş bir projede de aynı biçimde görünür — kaynağı ayırt etmez. `never built` artık hem diskte hiç çıktı olmadığı hem de bu aracın o projeyi hiç derlememiş olduğu durumu birlikte kapsar. `affected` artık yalnız bağımlılık değişimini değil, beslenen çıktının havuzdaki kopyasının kendi derleme kanıtıyla uyuşmadığı durumu da kapsar. Native title'lara üç yeni gerekçe eklendi: `Up to date — built outside this tool 5m ago` · `Its own files are newer than its build output` · `Its copy in the shared folder does not match its build output`.
- **Statü tablosu (§5).** `Succeeded` satırı artık kaynağını da söylüyor: aracın kendi derlemesi ya da dışarıda yapılmış bir derleme — ikisi de aynı yeşili, aynı glyph'i, aynı metni taşır.
- **Bilinçli karar (§8).** v1.20.0'ın "renk çıktının durumunu anlatır" ilkesine ek: çıktının tarihi tek başına güncel demeye yetmez — araç kendi derlediğinde kanıt içerikten, başkasının derlediğinde kanıt zamandan gelir.

## v1.21.0 — 2026-09-18

**Tek ağaç ve branch: worktree/in-place ayrımı kalkar, branch değişimi ve dışarıdaki git işlemleri kendiliğinden yansısın — kullanıcı isteği 2026-09-17 (spec §1 kararlar 1, 7-13, 20-23, §6, §8).**

- **Worktree ekrandan kalktı (§2.7, §2.8).** Action bar'daki `worktree: off ▾` chip'i ve Worktree popover'ı (açma/kapama, hedef listesi, silme) kalktı. Araç artık kullanıcının çalışma ağacında derler; Sync de Build de aynı ağacı görür. Branch chip'i `checkout eder` — seçim gerçek `git checkout <branch>`, ayrı bir "worktree'de derle" modeli yok.
- **Branch popover, checkout ve kirli ağaç (§2.8).** Dipnot `Switching checks the branch out in your working tree.`; aktif olmayan branch seçilince eskiden zorunlu olan worktree/boot/`Sync required` anlatısı kalktı. Kirli ağaçta davranış Settings → General'daki yeni anahtara göre: **Stop** (varsayılan) → konsola uyarı, hiçbir şey değişmez, konsol temizlenmez; **Stash and switch** → `git stash push` (mesaj branch'leri adlandırır), yeni bölüm açılır, konsola stash satırı + checkout. Geri uygulama kullanıcının — araç stash'i ne gösterir ne geri uygular.
- **Git işlemi yarıdayken kilit (§2.7, §3.9).** `index.lock` / `MERGE_HEAD` / `rebase-merge` / `rebase-apply` / `CHERRY_PICK_HEAD` / `REVERT_HEAD` sürerken branch chip'inde 5px amber nokta; checkout ve pull kilitli, kendiliğinden Sync bekler. `N behind` chip'inin görünürlüğünden "başka branch seçiliyken (worktree modu) chip yok" koşulu düştü — geride olunan her an görünür.
- **Sync artık kendiliğinden de çalışır, konsol bölüm kuralına göre davranır (§3.1, spec §6.2).** Yeni bölümü yalnız **kullanıcının başlattığı bir işlem** (Sync, Build/Rebuild/satırdan/Resolve, Clean/Optimize, Pull) veya **branch değişimi** açar — konsol temizlenir. **Commit** ve **pencereye dönüş** (5 sn eşiği) kendiliğinden Sync'tir: konsolu/olay akışını temizlemez, listeyi kaydırmaz, reveal oynatmaz, fetch yapmaz; olay akışına tek satır düşer. `.git\logs\HEAD` izlenir; checkout, commit, pull, reset görülünce tetiklenir, HEAD ve branch adı aynıysa çift Sync atlanır. Koşu sırasında branch değişirse nazik durdurma (değişim anındaki proje sonucu deftere yazılmaz); koşu bitince yeni bölüm özet satırıyla açılır.
- **State (§4).** Branch artık ayrı bir UI seçimi değil — `.git`'ten okunan gerçek aktif branch; `worktree {on|forced, chosen|auto}` kalktı. Eklenenler: `gitOpInProgress: bool` (branch chip'teki amber nokta, checkout/pull kilidi) ve kalıcı ayar `stashOnBranchSwitch: bool` (varsayılan `false` = *Stop*; Settings → General → BRANCHES, §2.9).
- **Bilinçli kararlar (§8).** v1.16.0'ın "aracın üstlenebileceği tek git yazma işlemi" ilkesi genişledi: mutasyon yüzeyi artık **üç kullanıcı eylemi** — ff-only pull, branch checkout, kirli ağaçta stash push; üçü de yalnız kullanıcı tıklamasıyla çalışır. Merge commit'i, rebase, force-push, otomatik pull/checkout/stash hâlâ YOK.

## v1.20.0 — 2026-09-18

**Renk modeli "son işlemin hikâyesi"nden "çıktının kümülatif durumu"na döner: neden — Sync sonrası neyin güncel olduğu renkten okunmuyordu, toplam görünmüyordu (kullanıcı ölçümü).**

- **Renk kuralı yeniden yazıldı (§2.3).** Görsel durumlar artık **bilinmiyor** (kesikli border, gri küp — yalnız Sync yokken) · **güncel** (yeşil) · **derlenecek** (gri) · **bozuk** (kırmızı, kanıtlı) · **işaretli** (amber) · **derleniyor** (amber + beads) · **sonuç**; sonuç bir sonraki koşuya kadar durumda yazılı kalır. Döngü üyesinde küp **her zaman** amber (çerçeve kendi durumunu taşır, küp döngü üyeliğini) — Resolve ile derlense de amber kalır.
- **Sync artık renk verir (§3.1).** Eskiden "hiçbir şey renklenmez, herkes başlangıç modunda kalır" idi; şimdi Sync sonrası her satır kendi durum rengindedir. Bilinmiyor (eski adıyla "başlangıç modu") artık yalnız hiç Sync yapılmadan önce görülür.
- **Karar etiketi sözlüğü sadeleşti (§2.4).** `modified` · `modified · local` · `affected` · `never built` · `failed · 2h` · `up to date · 2h`; yuva ölçüsü değişmedi (min 134px). `failed · retry` kalktı — "yeniden denenecek" sözünü sarı dalga zaten veriyor. Uyarı üçgeni kümülatif: defterdeki bağımlılık notu durdukça durur, bir sonraki koşu tek başına silmez. (Uygulamada görülen üç parçalı etiket `affected · up to date · 2h` bu tasarımın hiçbir sürümünde yer almadı — koddaki ayrı bir davranıştır, tasarımın parçası değildir.)
- **`—` (skipped) glyph/chip kalktı (§2.3, §2.7, §5).** Tire artık yalnız run-story yüzeylerinde (event stream'deki atlama satırı, konsol başlığındaki koşu sonucu) görünür; liste satırı, graf node'u ve sayaç chip'leri gibi durum yüzeylerinde hiç yer almaz — atlanan bir satır kendi standı rengini taşımaya devam eder, koşunun "N skipped" özeti ribbon'da kalır.
- **Action bar sayaç seti (§2.7).** `Σ · building · ✓ · ○ · ✗ · ⚠` (chip'te glyph + sayı; `up to date` / `to build` / `failed` sözcükleri tooltip'te); `⚠` artık döngü ile bekleyen bağımlılık notunu birlikte sayar, eski ayrı `▲` dep-affected chip'i kalktı.
- **Statü tablosu ve kararlar güncellendi (§5, §8).** Yeni satır `bozuk (kanıtlı)` — yalnız derleyici hatası (exit ≠ 0) kanıttır, timeout/Stop kanıt sayılmaz ve kırmızıya çevirmez, gri `never built` kalır. §8'deki v1.11.0 ilkesi ("renk yalnız son işlemin hikâyesi") v1.20.0'a taşındı: tek kanal ilkesi korunur, ayrı will-build noktası ve turuncu döngü işareti geri gelmez.

## v1.19.0 — 2026-09-16

**Üç dialog (Settings · What's new · About) yeniden tasarlandı: ortak kabuk, daha okunur ve ferah yerleşim, ayarların genişleyebileceği bir yapı (§2.9, §2.10, §2.11).**

- **Ortak dialog kabuğu (`DialogShell`).** Üçü de aynı kutuyu paylaşıyor: 18px gutter, aynı başlık/footer ritmi, `overflow: hidden`, sabit yükseklik + `maxHeight: calc(100% - 48px)` ve flex gövde. Sonuç: bölüm/sekme değişince dialog zıplamıyor, üç pencere aynı aileye ait görünüyor. Ölçüler içeriğin büyüme yönüne göre: **Settings 880×576** (iki panel), **What's new 720×600** (liste zamanla uzar), **About 620** (gövde sabit 284px).
- **Settings artık sol raylı iki panel (§2.9).** 196px bölüm rayı: **General · Workspace · External projects · Layers**; ray `surface` zemin + `border-subtle` ayraç, aktif satır `surface-overlay` + `text-primary` (accent statüde kalır), sağda mono sayaç. Her sayfa tek konu taşıyor ve **tek satır** açıklamayla açılıyor — eski 2–4 satırlık paragraflar ve bölüm başlığına sıkıştırılmış kontrol kalktı.
- **Genişleyebilir ayar satırı.** `ToggleRow`: solda ad + tek satır açıklama, sağda kontrol, satırlar arası hairline. Yeni ayar eklemek `GENERAL_GROUPS`'a bir satır; sayfa düzenine dokunulmuyor. Bağımlı satır üst anahtar kapalıyken %45 + `pointer-events: none`.
- **General bölümü (yeni).** STARTUP: `Start with Windows` · `Start minimized to tray` (üstteki kapalıysa disabled) · `Close to tray`. BUILD: `Pull before build`. NOTIFICATIONS: `Show notifications` — bildirimler bütün olarak açılıp kapanır (yalnız-hata kuralı istenmedi). Kalıcılık `delta-bo-general-v1` (`{startWin, startTray, closeTray, notify}`, varsayılan `{false, false, true, true}`), export/import JSON'unda `general` bloğu, kaydetmede değişen her anahtar için konsola tek dim satır.
- **Pull before build General'a taşındı.** Davranış (kopya başına tek komut, `git pull --ff-only` / `tf get`, konsol ve stream satırları) ve kalıcılık anahtarı v1.15.0'dan DEĞİŞMEDİ — yalnız yeri değişti; External projects sayfası nereye gittiğini ve açık/kapalı olduğunu yazıyor, ghost butonu rayı General'a alıyor. **v1.15.0'ın "bölüm başlığı satırında switch" kararı bununla geçersiz.**
- **`Load sample layers` kaldırıldı.** `Add layer` artık **boş** satır ekler; ad ve desen alanları **placeholder** gösterir ve bunlar ürüne özel değil, standart katman iskeleti: `Core` · `Infrastructure` · `Domain` · `Services` · `Api` · `Client` (`^MyApp\.(Core|Common)\.` biçiminde desenlerle, satır indeksine göre dönerek). Harici proje placeholder'ı da genelleşti (`C:\src\shared\MyApp.Common\MyApp.Common.csproj`), Workspace placeholder'ı `D:\src\myapp`.
- **Save neden kapalı — footer'da tek satır.** `Repository root is required` · `Every external project needs a path` · `Every layer needs a name` · `Check the highlighted pattern`. Kullanıcı hangi sayfada olduğuna bakmadan sebebi görüyor; export/import geri bildirimi geldiğinde onun yerini alıyor. First run Workspace'te, diğer açılışlar General'da başlıyor.
- **What's new: sürüm kimliği sol sabit sütunda (§2.11).** 2 kolonlu grid (84px + 1fr): solda sticky sürüm + tarih + `INSTALLED` çipi, sağda kategori blokları. Tarihler artık scrollbar'ın dibinde değil, her sürümde aynı sütunda; maddeler 1.62 satır yüksekliği, 10px satır aralığı ve 500px ölçüyle okunuyor. Sürümler arası boşluk 14 → 22px. Başlıktaki iki satırlı `INSTALLED VERSION` bloğu tek mono çipe indi.
- **About sadeleşti (§2.10).** `Third-party` sekmesi kaldırıldı. Sıra **About → Environment → Shortcuts**, F1 About'ta açıyor: kısa bir tanım paragrafı + `Version` / `Engine` / `Copyright` + `What's new in <sürüm>` butonu. Environment iki caps gruba ayrıldı (RUNTIME · PATHS) ve uygulama/engine sürümü oradan çıktı; Shortcuts BUILD · APPLICATION olarak gruplandı (metinler birebir).
- **Sekme bandı ayrıldı.** Alt çizgili `DS.Tabs` denendi ve **geri alındı** — kullanıcı segmentli kontrolü seçti (yeniden önerilmeyecek). Segment kendi bandında duruyor ve dikey ritim kullanıcıyla oturtuldu: tanım satırından segmente **26px**, segmentten hairline'a **14px**, hairline'dan içeriğe **14px**. Kutu 18px gutter'a hizalı (kontrol bir nesnedir), etiketin 11px iç boşluğu kontrole ait; "başlığa yapışık" hissi ve içeriğin aşağıda kalması giderildi. Renkler DS varsayılanında bırakıldı (amber yok).
- **Dosyalar:** `app/BuildApp.jsx` (`DialogShell` · `RailItem` · `PaneHead` · `ToggleRow` · `SettingsDialog` · `AboutDialog` · `NotesDialog` · `WhatsNew` yeniden yazıldı; `SAMPLE_LAYER_CFG` ve `ABOUT_THIRD` silindi, `LAYER_PLACEHOLDERS` · `SET_SECTIONS` · `GENERAL_GROUPS` · `DEFAULT_GENERAL` · `ABOUT_TABS` · `ABOUT_ID` eklendi), `app/release-notes.js`.

## v1.18.0 — 2026-09-14

**Graf panelinde işaretleme→koşu geçişi tek harekete indi; derleme göstergesi 200–300 projede görünür oldu (§2.5).**

- **Sıralı teslim dalgası (§2.5, §3.1):** işaretleme koreografisinin kuyruğu değişti. Grilerin vedasından sonra kapsam **hep birden** sönmüyor; **yandığı random sırayla** koşu seviyesine iniyor (`settleStagger` = min(40, 700/(n−1)) ms/node, node başına 400ms ease-in-out) ve **koşu bu kuyrukla örtüşerek** başlıyor — ilk derlenen node, son node'lar hâlâ sönerken tutuşuyor.
- **Ölü bekleyiş ve ikinci sönüm kaldırıldı (§2.5):** eski `wait` (1860+W) ve `wait2` (2280+W) fazları ile kapsamın 0.45'te bekleyip koşu başında 0.13'e **atlaması** silindi. Kapsam `settle` fazında doğrudan koşu seviyesine (0.13) iniyor, böylece `marking` → `running` geçişinde hiçbir opaklık zıplaması yok. Yeni saatler: `wave` 440 · `hold` 440+W · `dimenv` 740+W · `settle` 1300+W · **koşu 2060+W** (eskiden 2520+W) — koreografi ~460ms kısaldı.
- **Beads bir tık kalın (§2.5):** nokta kalınlığı 1 → **1.6px**, aralık 3.4 → **3.2px**, tur 4200 → **2400ms**. Node yapısı, **küp glyph'i**, hover 1.7×, seçim border'ı, giriş/çıkış opaklığı (420/640ms) ve reduced-motion kuralı DEĞİŞMEDİ.
- **Yörünge hücreye kelepçeli (§2.5):** `bgap = max(0.8, min(2.8, (pitch − node − 1.6 − 2) / 2))`. Az projede kelepçe kapanmaz (yörünge 2.8px dışta, eski yer); 200–300 projede pitch düşerken yörünge hücrenin içinde kalır ve **mürekkep dahil komşu şeritle arada ≥2px boşluk** bırakır — paralel derlenen iki komşu node'un şeridi değmez. Yerleşim fonksiyonu bu hesap için artık `pitch` de döndürüyor.
- **Değerlendirilip İSTENMEYENLER (yeniden önerilmemeli):** kapsamın 0.45'te kalıp koşu seviyesinin yükseltilmesi · tek sürekli sönüm · sönümün koşudan sonra gelmesi ("ilk node'dan devralma") · derlenen node'un dolgusunun tam amber olması · yörüngenin node içine alınması · node ile küçülmeyen sabit boyutlu (komşuya taşan) yörünge · derlenen node'un 1.5× büyümesi · panel boyunca uzanan radar kılavuz çizgileri · küçük node'da küp glyph'inin kaldırılması.

### Uygulama sayıları — sıralı teslim dalgası

| Faz | Zaman (gerçek saat) | Kapsam (işaretli) | Kapsam dışı | Not |
|---|---|---|---|---|
| `neutral` | 0–440ms | düz gri, opaklık 1 | düz gri, opaklık 1 | önceki koşunun renkleri silinir; başlangıç modundaki kesikli border düze döner |
| `wave` | 440ms → 440+W | random sırayla amber (border + zemin + küp aynı 200ms geçiş, gecikme = `markOrder` × `markStagger`) | gri | `markStagger` = min(110, 1100/(n−1)) ms · W = `markStagger`×(n−1)+380 |
| `hold` | 440+W → 740+W | amber, opaklık 1 | gri, opaklık 1 | sarı-gri okuma anı (~300ms), hiçbir şey oynamaz |
| `dimenv` | 740+W → 1300+W | amber, opaklık 1 | 0.18'e söner (1120ms ease-in-out) | grilerin vedası |
| `settle` | 1300+W → koşu | **yandığı sırayla** 0.13'e iner: gecikme = `markOrder` × `settleStagger`, her node 400ms ease-in-out | 0.18 | `settleStagger` = min(40, 700/(n−1)) ms → kuyruk ≤700ms |
| koşu | **2060+W** | derlenen 1.0 (+ beads), sırada bekleyen 0.13 | 0.13 (koşu kuralı) | kuyruk hâlâ sönerken ilk node tutuşur; faz geçişinde opaklık zıplaması YOK |

`settle` hedefi ile koşu seviyesinin **aynı değer (0.13)** olması bu tasarımın çekirdeğidir: `marking` → `running` geçişinde CSS'in yeniden hesaplayacağı bir opaklık farkı kalmaz. 36 projede W ≈ 1465ms → koşu ~3.5s'de başlar (eski koreografide ~4.0s).

### Konsol başlığı ve `Back` satırı — v1.18.0'da DEĞİŞMEDİ, uygulama tarafı tasarımdan sapmış

Bu sürümde bu alana dokunulmadı; buraya yazılmasının nedeni WPF uygulamasındaki karşılığının tasarımla uyuşmaması. Aşağıdaki tarif **prototipteki hâlin birebir spesifikasyonudur** (§2.6 ve §2.8 ile birlikte okunmalı).

**Kabuk.** Konsol paneli başlığı, projeler ve event stream başlıklarıyla **aynı `PanelHead` kalıbıdır**: yükseklik **28px sabit**, yatay padding **10px**, öğeler arası **gap 8px**, zemin `--surface`, altında **1px `--border-subtle`** hairline, `user-select: none`. Başlık kendi yüksekliğini korur (`flex: none`), asla sarmaz. Sol blok → esnek boşluk (`flex: 1`) → sağ blok: yani sol grup sola, sağ grup sağa yaslıdır, ortada hiçbir şey yoktur.

**İki hâli var — aynı kabuk, farklı içerik:**

1. **Ana loglar (seçim yok).** Solda tek öğe: **`CONSOLE`** — 11px (`--text-2xs`), ağırlık 500, `letter-spacing: --tracking-caps`, **CAPS**, renk `--text-faint`. Sağda: satır sayacı `N lines` — Geist Mono 11px `--text-faint`. Başka hiçbir kontrol yok (arama, temizle, filtre, tab YOK).
2. **Bir proje seçili (proje logu).** Caps `CONSOLE` etiketi **kaybolur** (`label=''`) — başlık artık projenin kimliğini taşır. Soldan sağa, tek satırda, 8px aralıklarla:
   - **`Back` butonu** — DS `Button`, `variant="ghost"`, `size="sm"` (**yükseklik 24px**), solunda **12px Lucide `arrow-left`** ikonu (`stroke-width 2`, `currentColor`), yanında **`Back`** metni (Geist 13px, ghost buton rengi). **`margin-left: -6px`**: ghost butonun iç padding'i optik olarak geri alınır, böylece ikonun sol kenarı panelin 10px'lik padding hizasına oturur — bu −6px tasarımın parçasıdır, kırpma değil. Ghost hover kuralı: zemin bir yüzey adımı açılır, **boyut ve renk tonu değişmez**. Tıklama = **yalnızca seçimi bırakır** (`selected = null`); filtre, scroll konumu, graf zoom'u ve koşu durumu ELLENMEZ. Klavyede aynı işi **Esc** yapar (Esc sırası: What's new → About → Settings → popover → seçim).
   - **Tam proje adı** — Geist **Mono 12px**, `--text-primary`, `white-space: nowrap` + `overflow: hidden` + `text-overflow: ellipsis`: panel daraldığında kısalan **tek** öğe budur (`Back` ve sağdaki sayaç asla kırpılmaz). Kısaltma değil TAM ad (ör. `OSYS.Orchestration.Service.WorkOrder`).
   - **Statü glyph'i** — 13px; `building` iken dönen amber arc (spinner), diğer statülerde DS `StatusGlyph` (ince halka içinde tik/çarpı/tire/saat).
   - **Statü metni** — 11px, rengi statünün `-text` tonu (`DS.STATUS_META[status].color`): `Building` · `Succeeded` · `Failed` · `Skipped` · `Queued` · `Discovered`. Glyph + metin daima birlikte (renk tek başına taşımaz).
   - **Koşullu amber uyarılar** (yalnız varsa, sırayla): `⚠ dependency issue` — tooltip'i `Dependency issue: <projeler> — last successful output referenced`; `⚠ dependency cycle` — tooltip'i `In a dependency cycle`. İkisi de 8px üçgen + 11px metin, `--amber-text`, `cursor: default`, tooltip `side="bottom"`, gecikmesiz.
   - **Sağ blok:** **`Copy log`** ikon butonu (DS `IconButton size="sm"`, kopyala ikonu; yalnız logda satır varsa görünür) — başarıda ikon **1.4s yeşil ✓** olur ve tooltip `Copied` yazar; ardından **`N lines`** sayacı (Geist Mono 11px `--text-faint`).

**Davranış sözleşmesi.** `Back` ile ana loglara dönüş ve proje loguna giriş **aynı geçişi** oynar: konsol gövdesi tek parça **"tilt in"** — alt kenardan menteşeli `perspective(900px) rotateX(7deg) translateY(14px)` + opaklık 0'dan düze, **340ms ease-out** (§2.6). Başlık bu geçişe katılmaz, içeriği anında değişir. Proje logu **salt okunur** bir kesittir: canlı satır basımı, daktilo ve imleç yalnız ana loglarda yaşar; proje logunda en altta yanıp sönen imleç yoktur.

**WPF tarafında sık sapan noktalar (kontrol listesi):** başlık yüksekliğinin 28px'ten büyümesi · `Back`'in ikonsuz ya da çerçeveli (secondary) buton olması · `Back` yanında dikey ayraç/çizgi konması · caps `CONSOLE` etiketinin proje logunda da durması · proje adının kısaltılması ya da `Back` metninin kırpılması · statü metninin nötr griye düşmesi (statü rengi şart) · `Copy log` ve `N lines`'ın sola kayması (sağ blok sağa yaslı kalmalı) · −6px optik hizanın uygulanmaması.

## v1.17.0 — 2026-09-14

**Hover dili tek elden; katman başlığı gezinme kontrolü oldu; harici proje kartı sadeleşti (§2.4, §2.5, §2.6, §2.7, §2.9).**

### 1 — Alt barda tek hover dili (§2.7)

Bardaki kontroller üç farklı hover davranışı gösteriyordu: Sync (secondary buton) zemini bir adım açıyor, bakım ikonları saydamdan `surface-raised`e geçiyor, çipler zemin açıyor ama border ve metni sabit tutuyor, `Debug | Release` segmenti ise hiç yanıt vermiyordu. Tek kural geldi:

- **Nötr kontrol:** `neutral-700` (#2a2a30) zemin + `neutral-500` (#54545c) hairline + `text-primary` **metin ve ikon**. Üç kanal aynı anda değişir — bakım ikonlarında yalnız ikon, çiplerde yalnız metin beyazlayan eski karışıklık bitti. Statü glyph'leri (✓ ✗ —) kendi rengini korur.
- **Açık/aktif kontrol** (açık filtre çipi, açık branch/worktree popover`ı, yürüyen bakım görevi / Resolve): `amber-soft-hover` zemin + `amber` border — hover aktif okumasını EZMEZ.
- **Segment (`Debug | Release`):** yalnız seçili OLMAYAN seçenek hover alır (`surface-raised` zemin + `text-secondary`); seçili seçenek `surface-overlay`da durduğu için ikisi karışmaz.
- Boyut, radius ve renk tonu sabit (DS: hover = yüzey adımı, hue değişmez), geçiş 80ms, disabled kontrol hover almaz.
- **Build/Stop hariç:** kendi variant hover`ında kalır; bar`ın tek yüksek sesli kontrolü odur.

Durum tablosu (hover):

| Kontrol | Zemin | Hairline | Metin / ikon |
|---|---|---|---|
| Nötr kontrol (Sync · bakım ikonları · Σ/✓/✗/—/⚠ çipleri · `N behind` · branch · worktree · perf) | `neutral-700` #2a2a30 | `neutral-500` #54545c | `text-primary` (ikon dâhil) |
| Açık/aktif kontrol (açık filtre, açık popover, yürüyen görev/Resolve) | `amber-soft-hover` | `amber` | `amber-text` (değişmez) |
| Segment — seçili olmayan | `surface-raised` | — | `text-secondary` |
| Build / Stop | variant hover (amber-bright / danger) | variant | variant |

Notlar: bakım kutusundaki üç ikon butonun kendi border'ı yok (kutu ortak çerçeveyi taşır) — onlarda hover zemin + ikon olarak okunur, ölçü ve ayraçlar sabit kalır. Statü glyph'leri (✓ ✗ —) hover'da da **kendi statü rengini** korur: renk statü taşır, hover taşımaz.

**Denenen ve bırakılan:** ilk sürümde nötr hover `surface-overlay` (#202024) + `border-strong` (#3a3a42) idi — DS'in "bir yüzey adımı" kuralına birebir uyuyordu ama 42px'lik barda, `surface` (#141417) zemin üzerinde **fark edilmiyordu**; ayrıca yalnız buton metnini beyazlattığı için çip ikonları gri kalıyor, bakım ikonlarıyla aynı görünmüyordu. İki adımlık zemin + belirgin hairline + ikonun da beyazlaması üçünü aynı dile getirdi.

WPF karşılığı: bar kontrolleri için tek `Style`, iki trigger — `IsMouseOver` (nötr hover) ve `IsMouseOver + IsChecked/IsActive` (açık kontrolün amber hover'ı); Build/Stop kendi style'ında kalır.

### 2 — Katman başlıkları tıklanabilir (§2.4)

Yapışkan katman başlığı artık gezinme kontrolü: hover`da zemin `surface-raised`, alt çizgi `border`, metin `text-secondary`; imleç pointer, native `title` = `Jump to <layer>`. Tıklama (Enter/Space de) o grubun **ilk satırını yığılmış başlıkların hemen altına** getirir — hedef `scrollTop = ilk satırın offsetTop − (başlık indeksi + 1) × 24`, smooth; reduced-motion`da anında. 36 projede uzun listede katmanlar arası gezinme scroll çekmeden yapılır. **Seçim, filtre, konsol ve graf değişmez** (satırdan tetiklenen işlemlerin aksine): başlık yalnız scroll eder. Katman katlama (collapse) bu sürümde yok.

### 3 — Konsol ve event stream: imleç + satır hover (§2.5, §2.6)

- Konsol gövdesinin imleci **standart ok** (`default`): el işareti yalnız tıklanabilir öğelere ait, metin I-beam'i bu panelde istenmedi.
- Konsol satırı hover`da `surface` (#141417) zeminli tam genişlik bant (gövde padding`ini aşan −12px kenar boşluğu), 80ms — event stream satır hover'ıyla **aynı görünür adım** (ilk denemede `surface-base` ile daha soluktu). Uzun logda satır takibi kolaylaşır; renk kuralı ve hiza değişmez.
- Event stream`de hover artık **her satırda**: tıklanabilir (projeli) satır `surface-hover` (#1a1a1e), tıklanamaz satır bir adım daha sessiz `surface` (#141417). İmleç tıklanabilirde pointer (el), diğerinde default (ok) — tıklanabilirlik hover'dan okunur. Seçili satır (2px amber şerit + `surface-raised`) değişmedi.
- **İki panelde hover adımı eşittir:** konsol #060608 → #141417, stream #0e0e10 → #1a1a1e; aynı algısal sıçrama, aynı 80ms. Renk, hiza, satır yüksekliği ve metin seçilebilirliği hover'dan etkilenmez — bant yalnız göz takibi içindir.
- **Denenen ve bırakılan:** konsolda metin imleci (`cursor: text`, I-beam) — çıktı okunup kopyalanan bir alan olduğu için mantıklı görünüyordu ama uygulamanın geri kalanında el işareti "tıklanır", ok "tıklanmaz" demek; üçüncü bir imleç dili istenmedi. Konsol satır hover'ının ilk değeri `surface-base` (#0e0e10) stream'in yanında soluk kaldı.

### 4 — Harici proje kartından `Source` seçimi kalktı (§2.9)

Kart üç parçaya indi: grip + path input + sil. Git/TFVC seçimi ve `Source` kolon başlığı kaldırıldı (kullanıcı kararı) — çalışma kopyasının türü yolun kendisinden okunur, kullanıcıya sorulmaz. Açıklama metni buna göre: `… a folder, a solution or a project file. The working copy root is found from the path upwards, and whether it is Git or TFVC is read from there.` Pull davranışı ve komutları DEĞİŞMEDİ (§9 v1.15.0). Veri modeli ve `externalProjects[{path, vcs}]` JSON alanı korunur: dosyadan gelen `vcs` okunur, yeni kart Git varsayar.

### Dosyalar

- `app/BuildApp.jsx` — `bo-app-css`: `[data-bo-actions] [data-bo-hov]` hover kuralları + `.bo-cline` satır bandı · `GroupHead` bileşeni + `jumpGroup()` · `NarrLine` → `.bo-cline` · `StreamRow` hover kuralı · konsol gövdesi `cursor: default` · action bar kontrollerine `data-bo-hov` · Settings harici proje kartı (`Source` kaldırıldı) · `ABOUT_VERSION` → 1.17.0.
- `app/release-notes.js` — 1.17.0 maddeleri.
...
- **Denenen ve bırakılan yerleşimler** (deney dosyası sonradan silindi): (1a alt satır sağa yaslı · 1b gövde metninin altında checkbox + açıklama · **1c başlık satırı — seçilen** · 1d kart içinde proje başına `Pull` kolonu · 1e listenin üstünde çerçeveli kural satırı). Ayrıca alt bardaki `pull` chip'i denenip kaldırıldı (aşağıdaki karar notu).
...
- **What's new ikonu kesinleşti:** **sparkle** (tek 4 kollu yıldız, 1.7px stroke, 13px) — dişli ve ⓘ ile aynı optik ağırlık. Adaylar (a sparkle · b daire-içi yıldız · c bayrak · d kitap · e liste+ · f paket · g çan · h doküman · i mono sürüm pill'i); kullanıcı **a**'yı seçti.
...
- İkon turu: çok-yıldızlı *sparkles* → *megaphone* → **sparkle** (v1.13.1). Adaylar dişli · aday · ⓘ üçlüsüyle gerçek 13px boyutta denendi (deney dosyası sonradan silindi).
...
**Konsol imleci renk sıçraması (§2.5, §2.6).** Seçenekler (deney dosyası sonradan silindi): (1a mevcut · 1b statü paleti turu · 1c amber sıcaklık · 1d nötr↔amber · 1e tam spektrum · 1f kırpma dibinde sıçrama · 1g son satırın rengi). Kullanıcı **1f**'yi seçti; palet isteği üzerine dört statü rengi yerine **konsol satır paleti** kullanıldı.
...
**Cycle işareti grafta + başlangıç modu kesiksiz.** Kullanıcıyla mutabık deneme dosyasında karara varıldı (dosya sonradan silindi; son hâl: 2a Build · 2b Resolve · 2c yakın plan). Ana koreografinin süre, sıra ve renkleri DEĞİŞMEDİ.
...
**3 — Dosyalar.** `app/BuildApp.jsx` (GTONE/GCORE/vstate, graf node opaklığı, Row şerit+nokta, `ABOUT_VERSION 1.12.0`), `app/build-data.js` (startResolve kapsamı, Resolve finali), `app/release-notes.js` (1.12.0 maddeleri).

## v1.11.0 — 2026-08-27

**Renk sadeleştirme: tek statü kanalı + her işlem için tek koreografi.** İlke: **renk yalnız son işlemin hikâyesini anlatır.** Kullanıcıyla mutabık plan dosyasında karara varıldı (dosya sonradan silindi).
...
- Denenip **istenmeyenler**: göz kırpma ×2, tek nefes (2e), sıralı (derleme sırası) dalga. Varyant dosyası (Tur 1-4) sonradan silindi.
...
Koreografi **yalnız graf node'larında** yaşar; proje listesi bitişte sabit kalır (kullanıcı kararı). Hızlı kontrol koşusunda ve derlenen proje yoksa koreografi çalışmaz. Yeni bir işlem (`_beginOp`) koreografiyi anında keser. Denenip istenmeyenler: grilerin tek tek kırpması, kıvılcım/pop, çift/üç kırpma (varyant dosyası Tur 5-6; dosya sonradan silindi).
