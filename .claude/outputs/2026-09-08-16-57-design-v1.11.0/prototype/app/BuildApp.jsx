...
const GTONE = {
  ...
  fresh: { bd: 'var(--border-strong)', bg: 'var(--surface-raised)', dash: true },
...
/* tek statü kanalı: node border'ı ve içindeki küp AYNI rengi taşır (plan/cycle kanalı kaldırıldı) */
const GCORE = {
  discovered: 'var(--text-faint)', fresh: 'var(--text-faint)',
...
function vstate(eng, name) {
  ...
  if (st.status === 'discovered') {
    ...
    return st.fresh ? 'fresh' : 'discovered';
  }
  return st.status;
}
...
function Row({ p, eng, selected, onSelect, onNote, onAction, onStop, busy, isTarget, revealIndex, menuOpen, setMenu }) {
  ...
  let rowOp = 1, rowGlide = '360ms var(--ease-in-out)';
  /* Bitiş koreografisi (neon) YALNIZ graf node'larında — proje listesi sabit kalır. */
  if (mstep && eng.isMarked(p.name)) {
    if (mstep === 'settle' || mstep === 'wait' || mstep === 'wait2') { rowOp = 0.45; rowGlide = '440ms var(--ease-in-out)'; }
  } else if (mstep && (mstep === 'dimenv' || mstep === 'settle' || mstep === 'wait' || mstep === 'wait2')) { rowOp = 0.3; rowGlide = '1120ms var(--ease-in-out)'; }
  ...
  const STRIP = {
    fresh: 'var(--status-skipped-border)', discovered: 'var(--status-skipped-border)', marked: 'var(--amber)',
  ...
  return (
    <div role="row" tabIndex={0}
      ...
      {/* sol şerit: başlangıç modunda kesikli, işlem başlayınca düz — node'larla senkron (dalgada sıralı gecikme) */}
      <span aria-hidden="true" style={{
        width: isSel ? 3 : 2, alignSelf: 'stretch', flex: 'none', margin: '1px 0',
        background: vs === 'fresh' ? 'transparent' : STRIP[vs],
        backgroundImage: vs === 'fresh' ? 'repeating-linear-gradient(to bottom, var(--status-skipped-border) 0 3px, transparent 3px 7px)' : undefined,
        transition: 'width var(--duration-instant) var(--ease-standard), background ' + (mstep === 'wave' && eng.isMarked(p.name) ? '200ms var(--ease-standard) ' + ((eng.markOrder[p.name] || 0) * eng.markStagger) + 'ms' : '180ms var(--ease-standard)'),
      }}></span>
      <span style={{
        ...
        {/* statü noktası: şeritle AYNI rengi taşır (ayrı plan kanalı değil); başlangıç modunda kesikli halka */}
        <span aria-hidden="true" style={{
          width: 8, height: 8, borderRadius: '50%', flex: 'none', boxSizing: 'border-box',
          background: vs === 'fresh' ? 'transparent' : STRIP[vs],
          border: vs === 'fresh' ? '1.5px dashed var(--status-skipped-border)' : 'none',
          transition: 'background ' + (mstep === 'wave' && eng.isMarked(p.name) ? '200ms var(--ease-standard) ' + ((eng.markOrder[p.name] || 0) * eng.markStagger) + 'ms' : '180ms var(--ease-standard)'),
        }}></span>
...
function ListPanel({ eng, groups, selected, onSelect, onNote, onRowAction, onStopRun, filter, query, follow, revealKey, workspace, layerCount, onOpenSettings, onImportSettings }) {
  ...
  React.useEffect(() => {
    if (!menu) return;
    const h = (e) => { if (e.target.closest && e.target.closest('[data-bo-popover]')) return; setMenu(null); };
...
function StickyStrip({ eng, onSelect, onFilterFailed, workspace }) {
  ...
  if (!workspace) { text = 'Not configured — repository root not set'; color = 'var(--text-faint)'; }
    ...
    text = eng._fastCheck() ? '▸ Ready — everything looks up to date' : `▸ Ready — ${wb} changed · ${36 - wb} up to date`;
...
function AboutDialog({ open, initialTab, onNotesSeen, onClose, logoSrc, companyLogoSrc }) {
  const DS = window.DeltaBuildOrchestratorDS_eb0bd1;
  const [tab, setTab] = React.useState('Shortcuts');
  const [copied, setCopied] = React.useState(false);
  const tRef = React.useRef(null);
  React.useEffect(() => { if (open) { setTab(initialTab || 'Shortcuts'); setCopied(false); } }, [open]); // eslint-disable-line
  React.useEffect(() => { if (open && tab === "What's new" && onNotesSeen) onNotesSeen(); }, [open, tab]); // eslint-disable-line
  React.useEffect(() => () => { if (tRef.current) clearTimeout(tRef.current); }, []);
  if (!open) return null;
    ...
    <div className="ds-scrim-in" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}
      ...
      <div role="dialog" aria-modal="true" aria-label="About Build Orchestrator" className="ds-dialog-in"
        ...
        <div style={{ padding: '14px 18px 14px', borderBottom: '1px solid var(--border-subtle)' }}>
          <DS.Segment size="sm" options={['Shortcuts', 'Environment', 'Third-party', "What's new"]} value={tab} onChange={setTab} />
        </div>
...
function SettingsDialog({ open, cfg, root, setup, importOnOpen, onClose, onSave }) {
  const DS = window.DeltaBuildOrchestratorDS_eb0bd1;
  ...
  React.useEffect(() => { if (open) { setDraft(cfg.map((l) => ({ ...l }))); setRootDraft(root || ''); setMsg(null); setClearArmed(false); dragRef.current = null; setDragIdx(null); setDragOff(0); if (importOnOpen) setTimeout(() => { if (fileRef.current) fileRef.current.click(); }, 120); } }, [open]); // eslint-disable-line
  ...
  const doExport = () => {
    ...
    try {
      ...
      flash(true, 'Exported — build-orchestrator-settings.json');
  ...
  const onFile = (e) => {
    ...
    rd.onload = () => {
      try {
        ...
        if (!layers && !hasRoot) throw new Error('shape');
        if (hasRoot) setRootDraft(d.repositoryRoot);
        if (layers) setDraft(layers);
        flash(true, 'Imported — ' + (layers ? layers.length : 0) + ' layers' + (hasRoot ? ' · root set' : ''));
  ...
  const doClear = () => {
    if (!clearArmed) {
      setClearArmed(true);
      flash(false, 'Click again to clear root and all layers');
    ...
    setRootDraft('');
    setDraft([]);
    flash(true, 'Cleared — nothing is applied until you save');
  };
  ...
  return (
    <DS.Dialog open title="Settings" width={620} onClose={onClose} footer={
      <>
        ...
        <DS.Tooltip content={clearArmed ? 'Click again to clear — root and all layers' : 'Clear settings — empty the form'} side="top">
        ...
        <DS.Button variant="primary" disabled={!valid || !rootDraft.trim()}
          onClick={() => onSave(rootDraft.trim(), draft.map((l) => ({ name: l.name.trim(), rx: l.rx })))}>{setup ? 'Save and sync' : 'Save'}</DS.Button>
      </>
    }>
...
function BuildAppInner({ scene = 1, autoPlay = true, simSpeed = 1, logoBase = 'assets' }) {
  ...
  const applyScene = React.useCallback((n) => {
    const e = engRef.current;
    setBranchPop(false); setWtPop(false); setBuildMenu(false); setSettingsOpen(false); setAboutOpen(false); setFilter(new Set()); setSelected(null);
    setQuery('');
  ...
  const openWorkspace = (root) => {
    setWorkspace(true);
    resetEngine({});
    eng.say('cmd', 'workspace: ' + root + ' — 36 projects discovered');
    setRevealKey((k) => k + 1);
    eng.startSync();
    bump();
  };
    ...
    <div data-screen-label={'Scene ' + sceneMeta.id + ' — ' + sceneMeta.label} style={{
      ...
      <DS.TitleBar logoSrc={null} title="">
        ...
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 2, paddingRight: 8, flex: 'none' }}>
          ...
          <DS.Tooltip content={unseenNotes ? "About — what’s new in " + ABOUT_VERSION + " (F1)" : 'About — version, shortcuts and diagnostics (F1)'} side="bottom">
            <span style={{ position: 'relative', display: 'inline-flex' }}>
              <DS.IconButton size="sm" aria-label="About" active={aboutOpen} onClick={() => setAboutOpen(true)}><I.info /></DS.IconButton>
              {unseenNotes && !aboutOpen && (
                <span aria-hidden="true" style={{
        ...
        {running || eng.phase === 'marking' ? (
          ...
          <span style={{ position: 'relative', display: 'inline-flex', flex: 'none' }} data-bo-chip="1">
            ...
            <Popover open={buildMenu} width={276}>
              ...
              <BuildMenuItem icon={<I.rebuild />} title="Rebuild" desc="All 36 projects — cache ignored" kbd="Ctrl+F5"
      ...
      {/* ---- AYARLAR ---- */}
      <SettingsDialog open={settingsOpen} cfg={layerCfg} root={repoRoot} setup={!workspace} importOnOpen={settingsImport}
        onClose={() => { setSettingsOpen(false); setSettingsImport(false); }} onSave={saveSettings} />

      {/* ---- ABOUT ---- */}
      <AboutDialog open={aboutOpen} initialTab={unseenNotes ? "What's new" : 'Shortcuts'}
        onNotesSeen={() => { setUnseenNotes(false); try { localStorage.setItem(NOTES_SEEN_KEY, ABOUT_VERSION); } catch (e) { /* yok */ } }}
