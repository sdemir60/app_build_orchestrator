...
> **Paket sürümü: v1.11.0** · tarih: 2026-08-27 — değişenler için → [Sürüm geçmişi](#sürüm-geçmişi)
>
...
**Statü noktası (v1.11.0 — tek kanal):** 8px daire; şerit/glyph/node ile **aynı statü rengini** taşır (amber=işaretli/derleniyor · yeşil · kırmızı · gri). Başlangıç modunda dolgu yok, 1.5px kesikli `--status-skipped-border` halka. Ayrı bir will-build kanalı YOK — ortogonal amber/gri/hollow nokta kaldırıldı..
...
- Sağ: 3 layout ikonu (quad/list/focus, tooltip'li) · 1px dikey ayraç · dişli (Settings, tooltip: "Settings — repository root and layer definitions") · **ⓘ (About, tooltip: "About — version, shortcuts and diagnostics (F1)")** · pencere kontrolleri (min/max/close).
...
Node border'ı ve içindeki küp **aynı statü rengini** taşır; ayrı "plan" ya da "cycle" çekirdeği YOK. Görsel durumlar: **başlangıç** (kesikli border, gri küp — Sync sonrası/açılış) · **nötr** (düz gri) · **işaretli** (amber) · **derleniyor** (amber + beads) · **sonuç** (yeşil/kırmızı/gri). Grafta uyarı üçgeni yoktur.
...
- **Satır (36px, alt çizgi `border-subtle`):** soldan sağa:
  1. **Statü şeridi** 2px dikey (satırın tam sol kenarı; statü rengi). Seçiliyken 3px. **Başlangıç modunda kesikli** — `repeating-linear-gradient(to bottom, --status-skipped-border 0 3px, transparent 3px 7px)` (v1.11.0).
  2. **Statü noktası** 8px — şeritle **aynı rengi** taşır (v1.11.0; ayrı plan/cycle kanalı yok), başlangıç modunda kesikli halka. Tooltip taşımaz. İşaretleme dalgasında şerit ve node ile aynı gecikmeyle yanar.
  ...
  4. Sağ blok (min 118px, sağa yaslı): hover'da **4 ikon buton (v1.11.0)** — **play** (o projeyi derler — §3.8), **⋯** (satır menüsü: Build · Rebuild · Clean; satıra sağ tık da açar), "Reveal in Explorer" (klasör) ve "Open in Visual Studio" (kod ikonu). İkon butonları **tooltip taşımaz** (yalnız native `title` + `aria-label`); tıklayınca konsola dim not düşer; hover yokken mono 10.5px SHA — dirty: `a3f81c2 → b7e91d4` (`text-secondary`), clean: tek `b7e91d4` (`text-faint`). Succeeded olan projenin `curSha`'sı hedefe eşitlenir — SHA her satırda görünür, satırlar arası sıçrama yok (v1.7.0).
...
- **İşlem koreografisinde satırlar (v1.11.0):** işaretleme fazında node'larla senkron — kapsam dışı satırlar 0.3'e (1120ms), kapsam satırları 0.45'e (440ms) iner, ikisi aynı anda biter; koşu başlayınca tam opaklığa dönerler. **Bitiş koreografisi (neon) satırlara UYGULANMAZ** — koşu bitiminde liste sabit kalır (kullanıcı kararı).
...
- **Sol şerit (v1.11.0):** her satırda görünür — **başlangıç modunda kesikli gri**, işlem başlayınca düz gri, işaretlenince amber, bitişte sonuç rengi. Sync şeridi kesikli hâle döndürür (plan göstermez). Şerit 2px (seçilide 3px) ve **1px dikey iç boşluklu** — boşluk satır ayracı kadar: bitişik satırlarda tek kesintisiz çizgiye kaynamaz, ama araları da açılmaz.
...
- **Segment** (DS.Segment, sm): `Shortcuts | Environment | Third-party | What's new`. Segment satırının altında **1px `border-subtle` hairline** dialog genişliğinde uzanır — sekme şeridi ile içeriği ayırır. İçeriği çerçeveleyen kutu ya da 3D/oyuk (inset) kanal YOK: DS yapıyı hairline ile taşır, bevel/gölge kullanmaz; 620px dialogda kutu-içinde-kutu ağırlık yapıyordu. Gövde min-yükseklik 236px — tab değişince dialog zıplamaz.
...
- **What's new (v1.9.0)** — sürüm notları, en yeni üstte:
  - Sürüm başlığı: mono 13px/500 numara + (güncel sürümde) sessiz caps `CURRENT` etiketi (yalnız `text-faint` metin — zemin/çerçeve yok, amber rozet göze batıyordu) + sağa yaslı mono 11px tarih. Sürümler arasında 14px boşluk + 1px `border-subtle`.
  ...
  - **Son 3 sürüm açık**, gerisi ghost buton `Earlier versions (N)` altında katlı — tıkla, hepsi açılır (geri katlama yok; dialog yeniden açılınca 3'e döner). Tam geçmiş erişilir, ekran sade kalır.
...
- **Görülmemiş sürüm işareti (v1.9.0):** localStorage `delta-bo-seen-version-v1` ≠ `ABOUT_VERSION` ise title bar'daki **ⓘ** üzerinde 5px amber nokta durur ve tooltip `About — what's new in 1.9.0 (F1)` olur; About bu durumda **doğrudan What's new sekmesinde** açılır ve sekme görülünce nokta söner. Açılış toast'ı / karşılama pop-up'ı YOK — yönlendirme bu sekmeye yapılır.
...
- **boot:** repo var, Sync yapılmadı. Konsolda açılış satırı: `Build Orchestrator 2.4.1 — Osys.sln loaded (36 projects) · main`. Tüm satır/node'lar **başlangıç modunda** (kesikli).
- **Sync:** konsola `▸ git fetch origin main` → `HEAD b7e91d4 — computing osys-state diff` → `Sync complete — 7 changed projects, 14 to build` + `22 projects up to date (will skip)`. **Hiçbir şey renklenmez** (v1.11.0): herkes başlangıç modunda kalır, değişenlerde çift SHA görünür; şerit `Ready — 14 changed · 22 up to date`, işlem pill'i `SYNC`. Açılış = otomatik sync, kapat-aç hep bu moda döner.
...
- Succeeded olan projenin `will` alanı `clean` olur ve `curSha` hedefe eşitlenir (model; ayrı bir görsel kanal yok — v1.11.0).
...
- **Pass 2:** yalnız üyeler taze referanslarla yeniden derlenir (~0.45× süre) → yakınsar; `will=clean`, `curSha` hedefe eşitlenir. Sonuç normal succeeded; **döngü işareti yalnız amber uyarı üçgeni** olarak kalır (v1.11.0 — turuncu nokta/çekirdek yok); konsola dim not düşer.
...
- **Engine/VM durumu:** faz; proje başına `{status: discovered|queued|building|succeeded|failed|skipped, fresh: bool (başlangıç modu), will: dirty|clean|unknown (yalnız kapsam hesabı — görsel kanal DEĞİL), startAt, doneDur, depIssue: string[]|null, log[]}` + koreografi alanları (`marked`, `markStep`, `markOrder`, `markStagger`, `endStep`, `endOrder`, `endStagger`, `op`) + **kalıcı cycle üyelik kümesi** (statü DEĞİL — v1.7.0); willBuild kümesi (döngü üyeleri hariç); `resolveRun {list, pass, fin, total, startT}`; sayaçlar (building/succeeded/failed/skipped/queued, depIssueCount, cycle=üyelik sayısı); elapsed + ETA; anlatı satırları `{type, time, text}`; stream olayları `{kind, time, project?, text}`; aktif satır.
...
| başlangıç modu (`fresh`) | kesikli şerit + kesikli nokta + kesikli daire glyph / kesikli node border | `--status-skipped-border` | tooltip yok — Sync sonrası ve açılışta herkes bu moddadır |
...
| ~~will-build (ortogonal dot)~~ | **v1.11.0'da KALDIRILDI** — nokta ve node çekirdeği artık statü rengini taşır | — | plan bilgisi çift SHA metninde (`a3f81c2 → b7e91d4`) |
...
- Satırlar node'larla senkron söner/yerleşir (`rowOp`/`rowGlide`); `noReveal` ref'i reveal animasyonunun fill kilidini bırakır.
