...
(function () {
  ...
  SimEngine.prototype._blankState = function (pd, rng) {
    return {
      def: pd,
      status: 'discovered',
      fresh: true,               // başlangıç modu (soluk düz şerit + 4 yaylı nokta) — ilk derleme işlemiyle düşer
      will: 'unknown',           // dirty | clean | unknown
      curSha: sha7(rng || this.rng),
      dur: Math.round(pd.dur * (1.05 + (rng || this.rng)() * 0.5)),
  ...
  /* ---- build öncesi harici pull (alt bardaki 'pull' çipi) ----
  ...
  SimEngine.prototype._cleanSlnPlan = function () {
    ...
    return {
      ...
      finish: function (e, el) {
        ...
        e.allClean = false;
        e.willBuild = new Set(P.filter(function (pd) { return !e._isCycle(pd.name); }).map(function (pd) { return pd.name; }));
        P.forEach(function (pd) { const st = e.p[pd.name]; st.will = 'dirty'; st.fresh = true; });
        e.runStartT = null; e.doneAt = null; e.stoppedAt = null; e._finishedWB = 0;
        e._building = []; e._eta = null; e.lastFail = null; e.checkDur = null;
  ...
  window.DELTA_BO = {
    ...
    SimEngine, fmtDur, fmtClock, shortName, mulberry32, setExternal,
