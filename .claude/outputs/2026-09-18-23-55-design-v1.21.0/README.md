...
- Grup başlıkları caps 11px `text-dim`: **STARTUP** · **BUILD**. Gruplar arası 22px.
...
- **Ayar (Settings → General, kalıcı, v1.21.0):** `stashOnSwitch: bool` (varsayılan `false` = *Stop*; `true` = *Stash and switch*) — kirli ağaçta branch chip'inden değiştirmenin davranışını seçer (§2.8, §3.1).
...
- **State (§4).** Branch artık ayrı bir UI seçimi değil — `.git`'ten okunan gerçek aktif branch; `worktree {on|forced, chosen|auto}` kalktı. Eklenenler: `gitOpInProgress: bool` (branch chip'teki amber nokta, checkout/pull kilidi) ve kalıcı ayar `stashOnSwitch: bool` (varsayılan `false` = *Stop*).
