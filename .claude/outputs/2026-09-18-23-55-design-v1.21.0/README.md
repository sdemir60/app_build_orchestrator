...
> **Paket sürümü: v1.21.0** · tarih: 2026-09-18 — değişenler için → [Sürüm geçmişi](#sürüm-geçmişi)
>
...
> davranışları temsil eder. Bu sürümde yalnız metin değişti: worktree/in-place ayrımı kalktı, branch chip'i
> ana ağaçta gerçek checkout yapar, branch değişimi ve dışarıdaki git işlemleri kendiliğinden Sync'e yansır
> (§2.7, §2.8, §3.1, §3.4, §3.9, §4, §8, §9).
...
Node border'ı ve içindeki küp **aynı statü rengini** taşır; ayrı "plan" ya da "cycle" çekirdeği YOK. Görsel durumlar: **bilinmiyor** (kesikli border, gri küp — yalnız Sync yokken) · **güncel** (yeşil) · **derlenecek** (gri) · **bozuk** (kırmızı, kanıtlı) · **işaretli** (amber) · **derleniyor** (amber + beads) · **sonuç** yeşil/kırmızı duruma yazılır ve kalır — bir sonraki koşuya kadar sabittir, "son işlemin hikâyesi" değil çıktının o anki durumudur. **Döngü üyesinde küp her zaman amber** (`vstate` = `cyc` gri border · `cycskip` skipped border) — satırdaki uyarı üçgeninin grafik vekili; çerçeve kendi durumunu (bilinmiyor/derlenecek/derleniyor/sonuç) taşır, küp döngü üyeliğini sürekli işaretler. Grafta ayrı bir uyarı üçgeni yoktur.
...
- **v1.20.0:** renk ÇIKTININ DURUMUNU anlatır; tek kanal ilkesi korunur (şerit + nokta + glyph + node border + küp aynı renk). Ayrı will-build noktası ve turuncu döngü işareti GERİ GELMEZ; döngü yalnız amber küp + amber üçgen + log ile ifade edilir. Kırmızı badge eklenmez (kırmızı = kanıtlı bozuk: derleyici hatası, bugünkü imzayla eşleşiyor).
