...
window.DELTA_BO_NOTES = [
  {
    v: '1.19.0', date: '2026-09-16', items: [
      ['feature', 'Settings opens on a section rail — General, Workspace, External projects, Layers — so every page holds one topic and a new setting is one more row instead of a longer scroll.'],
      ['feature', 'General is where the switch-weight settings live: start with Windows, start minimized to tray, close to tray, pull before build, show notifications. They apply when you save and travel with an exported settings file.'],
      ['change', 'A setting is one row: its name, one line that explains it, the control on the right, a hairline to the next. The long paragraphs and the switch tucked into a section heading are gone.'],
      ['change', 'Pull before build now sits in General with the other switches. The External projects page says where it went and whether it is on, and jumps there.'],
      ['removed', 'Load sample layers is gone. Add layer inserts an empty row and the placeholders are a standard layer skeleton — Core, Infrastructure, Domain, Services, Api, Client — instead of names from one product.'],
      ['change', 'What’s new keeps each version and its date in a fixed left column that stays put while its notes scroll, and the notes themselves sit in a measured column with more air between them.'],
      ['change', 'About opens on what the app is: a short description, version, engine and copyright, with a link to the release notes. Environment groups runtime and paths, Shortcuts groups build and application keys.'],
      ['removed', 'The Third-party tab left About. It listed placeholder licences and nothing else.'],
      ['change', 'The tabs in About keep their segmented control, now with room above it and a full-width hairline below, so the identity block and the content it switches read as two separate things.'],
      ['change', 'The three dialogs share one shell — same gutter, same header and footer rhythm, a fixed height each (Settings 880×576, What’s new 720×600, About 620) — so nothing jumps when you change section or tab.'],
    ],
  },
  {
    v: '1.18.0', date: '2026-09-14', items: [
      ['change', 'The build indicator on a graph node is easier to follow: the dots travelling around the node are a notch thicker and complete a lap in 2.4 seconds instead of 4.2. The ring is clamped to the node’s own cell, so even at two or three hundred projects the ribbons of neighbouring nodes keep a clear gap — nothing overlaps, and the node, its cube and the selection ring are unchanged.'],
      ['change', 'The console header is unchanged, and this release states it in full so the implementation can be brought back in line: the header keeps the panel height of 28px, the CONSOLE label disappears while a project log is open, and the row then reads Back — full project name — status glyph — status word, with Copy log and the line count held to the right edge. Back only drops the selection; the filter, the scroll position and the graph view stay as they were, and Esc does the same.'],
      ['change', 'The hand-over from marking to the run is one continuous movement: once the projects that will be skipped have faded, the marked ones dim in the same random order they lit up, and the first project starts building while the last of them are still dimming. The idle pause before a run and the sudden second fade at its start are gone.'],
    ],
  },
  {
    v: '1.17.0', date: '2026-09-14', items: [
      ['change', 'Every control in the status bar now answers the mouse the same way: the surface opens, the hairline brightens, and the label and its icon turn white together — a status glyph keeps its own colour. A control that is switched on — an open filter, an open picker, a running maintenance task — deepens its amber instead, so hovering never hides what is on. Build and Stop keep their own hover; they are the loud controls of the bar.'],
      ['feature', 'A layer heading in the project list is a jump target: hovering lifts it, clicking brings the first project of that layer right under the stacked headings. Nothing else moves — the selection, the filter and the console stay where they were.'],
      ['change', 'The line under the pointer lifts one step in the console too, the same step as in the event stream, which makes a long log easier to follow. The cursor stays an arrow; the hand belongs to what can be clicked.'],
      ['change', 'Every event stream row answers the mouse, not only the ones that carry a project; a project row lifts one step further and keeps the hand cursor, so what can be clicked stays obvious.'],
      ['removed', 'External project cards no longer ask for Git or TFVC. A card is a path, and whether the working copy is Git or TFVC is read from the path. Imported settings files are still read as before and the pull step is unchanged.'],
    ],
  },
  {
    v: '1.16.0', date: '2026-09-10', items: [
      ['feature', 'The status bar has a behind chip next to the branch: when the fetch on Sync finds new commits on the remote, it shows how many commits the local HEAD is behind. Clicking it fast-forwards — fetch, a check that the branch is only behind, then merge --ff-only. Never a merge commit, never a rebase, never on a dirty working tree, and only on the active branch; the tool still never pulls on its own.'],
      ['change', 'A project row says why it will be built instead of showing a commit pair: modified when its own files changed, affected when only a dependency changed, never built when there is no output on disk, failed · retry after a failed build. Everything that will be skipped says up to date with the age of its last build.'],
      ['change', 'What decides a build is the content of a project’s own files on disk — one rule for the repository and for external roots alike. The row label is that fact, so it reads the same before, during and after a run.'],
      ['change', 'The Sync line in the console states the distance from the remote: HEAD a3f81c2 · 3 commits behind origin/main, or up to date with origin/main. When the fetch is degraded no distance is claimed and the chip stays away.'],
      ['change', 'A successful pull is written to the console and followed by an automatic Sync, so the plan is computed against the new HEAD.'],
      ['fix', 'The External projects description no longer claims the cards set the build order: card order is the order the working copies are updated, and external projects build among themselves in dependency order.'],
    ],
  },
  {
