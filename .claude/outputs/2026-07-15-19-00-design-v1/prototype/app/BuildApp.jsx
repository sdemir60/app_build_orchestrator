...
function BuildAppInner({ scene = 1, autoPlay = true, simSpeed = 1, logoBase = 'assets' }) {
  ...
  const applyScene = React.useCallback((n) => {
    const e = engRef.current;
    setBranchPop(false); setWtPop(false); setBuildMenu(false); setSettingsOpen(false); setFilter(null); setSelected(null);
    setQuery('');
  ...
  return (
    <div data-screen-label={'Scene ' + sceneMeta.id + ' — ' + sceneMeta.label} style={{
      ...
      {/* ---- TITLE BAR ---- */}
      <DS.TitleBar logoSrc={logoBase + '/delta-logo-dark.svg'} title="Build Orchestrator">
        <span style={{ marginRight: 'auto', display: 'inline-flex', alignItems: 'center', gap: 8, paddingLeft: 6, minWidth: 0 }}>
