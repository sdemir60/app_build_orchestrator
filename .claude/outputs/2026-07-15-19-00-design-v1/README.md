...
- Sol: Delta logosu (dark varyant, 15px) + "Build Orchestrator" başlığı; ardından mono 11px `text-dim` bağlam: `OSYS · main` — worktree aktifse `· main-2` eklenir (`text-faint`). Repo yokken: `no repository`.
- Sağ: 3 layout ikonu (quad/list/focus, tooltip'li) · 1px dikey ayraç · dişli (Settings, tooltip: "Settings — layer definitions") · pencere kontrolleri (min/max/close).
...
- Kısayollar: **F5 = Build**, **Ctrl+F5 (veya Shift+F5) = Rebuild**, Esc yukarıdaki gibi. (Gerçek uygulamada global hotkey RegisterHotKey — plan v6.)
...
- `prototype/assets/delta-logo-dark.svg` — title bar wordmark (15px yükseklik, amber yay korunur).
- `prototype/assets/delta-app-icon.svg` — pencere/taskbar ikonu (koyu zemin + amber D).
