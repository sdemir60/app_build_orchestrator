# Build Orchestrator — Yeni Plan (v2)

> Bu doküman, orijinal 11 bölümlük prompt'un ([2026-06-27-21-40-build-orchestrator-orijinal-prompt.md](2026-06-27-21-40-build-orchestrator-orijinal-prompt.md)) sıfırdan yeniden kurgulanmış hâlidir. Mevcut durum analizinin ([../summaries/2026-06-27-21-52-build-orchestrator-mevcut-durum-analizi.md](../summaries/2026-06-27-21-52-build-orchestrator-mevcut-durum-analizi.md)) ortaya çıkardığı borçlar (özellikle §6.1 process kontrolü) bu planda baştan doğru kurgulanmıştır.

**Tarih:** 2026-06-27 22:46
**Karar süreci:** Brainstorming ile, kullanıcı onaylı. Aşağıdaki temel kararlar diyalogla verildi.
...
App (UI)  ── outer Job [JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE]
   └─ Supervisor (CREATE_SUSPENDED → AssignProcessToJobObject → ResumeThread)
         └─ inner Job [JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE]
...
- Projeler sabit ortak **OutDir**'e DLL üretir; container oradan okur. **Bu davranış korunur, OutDir'e DOKUNULMAZ.**
- **Final çıktı (bin) branch'e göre izole EDİLMEZ.** Bilinçli tercih: "şu an hangi branch derlendiyse onun DLL'i container'da geçerlidir."
- Yalnızca **ara çıktı (obj / `BaseIntermediateOutputPath`)** worktree içinde izole tutulur. İzolasyon anahtarı **proje Id (tam yol)** — Name değil (eski bug fix: aynı isimli projeler obj paylaşamaz).
- **Sonuç:** OutDir paylaşımlı → DLL timestamp "değişti mi" kararında GÜVENİLMEZ. Karar yalnız **kaynak sinyaline** (commit + local diff) dayanır.
...
- `build-state.json` → `branch::projectId` anahtarlı; başarısızlıkta eski commit korunur (tekrar dener).

### Paralellik
Bağımsız projeler eşzamanlı. Paralel derece **bizde** (Supervisor) yönetilir; derece + process priority **performans moduna** (Full/Balanced/Light) bağlı. Priority, Job Object üzerinden uygulanır.

### Branch yönetimi (worktree)
- Açılışta **kullanıcının aktif branch'i** otomatik seçili.
- Başka branch seçilse bile **VS'de açık working tree DEĞİŞMEZ.** Orchestrator branch'i `%LOCALAPPDATA%\BuildOrchestrator\worktrees\<branch>\` havuzunda hazırlar/günceller ve orada derler. Sonuç ortak OutDir'e düşer.
...
## 7. UI / UX (tek pencere — §7 korunuyor, eksik imzalar tamamlanıyor)
...
**Animasyonlar (UI'ı KİLİTLEMEZ; eski eksikler tamamlanıyor):**
...
- Aktif kart: pulse border + hafif scale + öne gelme (carousel hissi). Fail'de kısa shake, success'te glow, skipped'te fade + **desaturate**.
- **Auto-focus:** aktif kart görünüre kayar; manuel scroll'da takip durur, ~2 sn idle'da devam (eski kodda "her ProjectStarted'da zorla en üste atlama" yerine).
- **ReducedMotion gerçekten tüm animasyonlara bağlanır** (eski bug: hiçbirine bağlı değildi). Açıkken animasyonlar minimuma iner.

---

## 8. Test Stratejisi (§10 — process testleri artık first-class)

- **Unit (Core):** graph extraction, topolojik sıra, cycle tespiti, dosya→proje eşleme, **incremental kararı (commit-delta tetikleyici izole)**, Safe/Fast propagation, scanner ignore-dir.
...
- **Integration:** çoklu-solution örnek workspace → Sync, Build, Rebuild, branch switch (worktree), Stop.
- **Performans:** 500+ kart akıcı scroll; cache-hit'te hızlı sync; paralel build ölçülebilir kazanç; log akışında UI bloklanmaz.

---

## 9. Supervisor ↔ UI Sözleşmesi (§8 — temiz tutulur)

- **Komutlar:** `syncWorkspace(rootPath)`, `reanalyze()`, `listBranches()`, `selectBranch(branch)`, `startRun(mode, branch, config, dependentMode)`, `stopRun(runId)`, `openPath(projectId)`, `openInVS(projectId)`.
- **Eventler:** `syncProgress`, `syncCompleted`, `runStarted`, `projectStarted`, `projectLog`, `projectSucceeded`, `projectFailed`, `projectSkipped`, `runCompleted`, `runCancelled`, `error`.
- **Tipler:**
...
| **3** | **Incremental Build**: commit/diff/status, build-state, Safe/Fast, worktree + obj izolasyonu, Skipped. | Projenin asıl zekâsı. |
| **4** | UX polish + config: imza animasyonları, ReducedMotion, console toggle, filtre/Stop morph, config ekranı (LogLevel/ReducedMotion **dahil**), tray, autostart, single-instance. | Cila + eski config bug fix. |
| **5** | Perf modları (Full/Balanced/Light derece + priority), 500–1000 kart perf doğrulama, basit `dotnet publish` paketleme. | Ölçek + dağıtım. |
...
- Performans modu: **Full Power / Balanced / Light** (paralel derece + process priority).
...
- **İçinde:** tek repo, shell-out derleme, nested-Job process kontrolü, sync/graph/cache, rebuild + incremental, worktree, tam UI/UX + animasyonlar, config, tray/autostart/single-instance, perf modları, unit+process+integration test, basit `dotnet publish`.
- **Dışında (sonraya):**
...
- Ortak OutDir korunur; final DLL'ler branch'e göre izole edilmez; "değişti mi" kararı yalnız commit + local diff.
- Kullanıcı VS'de aynı projeleri Orchestrator ile eşzamanlı derlemez.
- Varsayılanlar: Debug, Safe, worktree, Errors-only, Full Power, Reduced Motion kapalı.
...
Bu plan onaylandıktan sonra **writing-plans** ile Iteration 0 için detaylı, adım adım uygulanabilir implementation plan çıkarılacak.
