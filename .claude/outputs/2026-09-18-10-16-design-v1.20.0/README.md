...
| Sürüm | Tarih | Konu |
|---|---|---|
| **v1.20.0** | 2026-09-18 | **Renk çıktının durumunu anlatır**: "son işlemin hikâyesi" ilkesi terk edildi — Sync sonrası satırlar durum renginde (yeşil/gri/kırmızı), `bilinmiyor` yalnız hiç Sync yokken görülür · karar etiketi sözlüğü sadeleşti (`modified · local` eklendi, `failed · retry` ve üç parçalı etiket kalktı) · `—` (skipped) glyph'i ve chip'i kalktı, yalnız run-story yüzeylerinde kalır · döngü küpü her zaman amber · `⚠` chip'i eski `▲` dep-affected chip'ini yuttu |
...
- **Karar etiketi sözlüğü sadeleşti (§2.4).** `modified` · `modified · local` · `affected` · `never built` · `failed · 2h` · `up to date · 2h`; yuva ölçüsü değişmedi (min 134px). `failed · retry` ve üç parçalı etiket (`affected · up to date · 2h`) kalktı — "yeniden denenecek" sözünü sarı dalga, kalıcı üçgeni de uyarı slotu zaten veriyor. Uyarı üçgeni kümülatif: defterdeki bağımlılık notu durdukça durur, bir sonraki koşu tek başına silmez.
