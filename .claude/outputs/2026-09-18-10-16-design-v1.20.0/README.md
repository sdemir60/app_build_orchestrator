...
3. 1px dikey ayraç.
4. **Sayaç chip'leri — v1.11.0: ÇOKLU filtre** (`filter` bir Set; çipler bağımsız açılır-kapanır, seçili küme **VEYA** ile birleşir — seçili chip'ler **durum filtreleridir**; arama ile VE). Aktif çip **kendi statü renginde** yanar. Set (v1.20.0): `Σ 36` (tümü/filtre temizle) · spinner+`4` (building; boşken gri nokta) · `✓ 14 up to date` · `○ 17 to build` · `✗ 5 failed` · `⚠ 3` (döngü ve bekleyen bağımlılık notu birleşik, filtre `warn`, tooltip `In a dependency cycle or waiting on a dependency — filter`). `— (skipped)` chip'i **KALKTI** — "atlanmak" bir durum değil, satır kendi standı rengini (güncel/derlenecek) taşımaya devam eder; koşunun "N skipped" özeti ribbon'da kalır (§2.2). Ayrı `▲` dependency-affected chip'i de kalktı, `⚠`'a birleşti. `⚠` yalnız listede karşılığı varken görünür, boş/gri chip taşımaz. Aktif filtre chip'i vurgulu.
5. Esnek boşluk.
...
- **Action bar sayaç seti (§2.7).** `Σ · building · ✓ up to date · ○ to build · ✗ failed · ⚠`; `⚠` artık döngü ile bekleyen bağımlılık notunu birlikte sayar, eski ayrı `▲` dep-affected chip'i kalktı.
