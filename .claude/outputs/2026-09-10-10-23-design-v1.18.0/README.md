...
> **Paket sürümü: v1.18.0** · tarih: 2026-09-14 — değişenler için → [Sürüm geçmişi](#sürüm-geçmişi)
>
...
> **v1.17.0:** harici proje kartından **`Source` (Git/TFVC) seçimi ve `Source` kolon başlığı kaldırıldı** (kullanıcı kararı). Kart artık grip + path input + sil. Çalışma kopyasının türü **yolun kendisinden** okunur; bölüm açıklaması bunu söyler: `… a folder, a solution or a project file. The working copy root is found from the path upwards, and whether it is Git or TFVC is read from there.` Pull davranışı DEĞİŞMEDİ (§9 v1.15.0): kopya başına tek komut, Git `git -C "<path>" pull --ff-only`, TFVC `tf get "<path>" /recursive`. Veri modeli ve export/import alanı (`externalProjects[{path, vcs}]`) korunur — dosyadan gelen `vcs` okunmaya devam eder, UI`da seçilemez.
...
- **Kategori satır başına değil BLOĞA yazılır** (Keep a Changelog mantığı): 6px renkli kare (radius 1) + caps 11px `text-dim` başlık, altında 13px maddeler 13px içeriden (4px aralık, `leading-snug`). Satır başında ikon/sigil/tooltip YOK — ikonlu ve diff-sigilli varyantlar denendi, blok başlığı seçildi (deney dosyası sonradan silindi).
