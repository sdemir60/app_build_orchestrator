...
function SettingsDialog({ open, cfg, extCfg, pull, root, setup, importOnOpen, onClose, onSave }) {
  const DS = window.DeltaBuildOrchestratorDS_eb0bd1;
  ...
  /* build öncesi pull — harici projelerin kendi bölümünde yaşar, listeyi yöneten kural */
    ...
    <DS.Dialog open title="Settings" width={760} onClose={onClose} footer={
      <>
        ...
        <DS.Button variant="primary" disabled={!valid || !rootDraft.trim()}
          onClick={() => onSave(rootDraft.trim(), draft.map((l) => ({ name: l.name.trim(), rx: l.rx })), ext.map((x) => ({ path: x.path.trim(), vcs: x.vcs === 'tfvc' ? 'tfvc' : 'git' })).filter((x) => x.path), pullDraft)}>{setup ? 'Save and sync' : 'Save'}</DS.Button>
      </>
    }>
      ...
      {/* bölüm başlığı satırı: solda etiket, sağda bölümün kuralı (pull) — gövde okuma akışına dokunmaz (1c) */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 6 }}>
        <span style={caps}>External projects</span>
        <span aria-hidden="true" style={{ flex: 1, height: 1, background: 'var(--border-subtle)' }}></span>
        <DS.Tooltip side="top" content="Every build starts by updating each external working copy — git pull --ff-only or tf get, one command per copy. Off: the copies are built as they are on disk.">
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 9, ...caps, cursor: 'default' }}>Pull before build
            <DS.Switch checked={pullDraft} onChange={setPullDraft} />
          </span>
        </DS.Tooltip>
      </div>
      <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-dim)', lineHeight: 'var(--leading-snug)', marginBottom: 12 }}>
        Projects outside the repository root — a folder, a solution or a project file. The working copy root is found from the path upwards, and whether it is Git or TFVC is read from there.
        They are built <strong style={{ color: 'var(--text-secondary)', fontWeight: 500 }}>before</strong> everything else; the rest follows the layers below. Card order only sets the order the working copies are updated — among themselves they build in dependency order.
      </div>
      {!ext.length && (
...
function BuildAppInner({ scene = 1, autoPlay = true, simSpeed = 1, logoBase = 'assets' }) {
  ...
  const saveSettings = (root, cfg2, ext2, pull2) => {
    setLayerCfg(cfg2);
    setExtCfg(ext2 || []);
  ...
  return (
    <div data-screen-label={'Scene ' + sceneMeta.id + ' — ' + sceneMeta.label} style={{
      ...
      {/* ---- AYARLAR ---- */}
      <SettingsDialog open={settingsOpen} cfg={layerCfg} extCfg={extCfg} pull={pullExt} root={repoRoot} setup={!workspace} importOnOpen={settingsImport}
