...
public class CopyLogTests
{
    ...
    public void CopyLog_button_visible_only_in_project_log_mode_with_lines()
    {
        ...
        header.ShowProjectLog(ConsoleHeaderRow.For("OSYS.Base", ProjectRowState.Succeeded, inCycle: false, depIssues: null, namePrefix: ""), 0);
        Assert.Equal(Visibility.Collapsed, header.CopyLogButton.Visibility); // log yok → copy yok

        header.ShowProjectLog(ConsoleHeaderRow.For("OSYS.Sales.Core", ProjectRowState.Started, inCycle: false, depIssues: null, namePrefix: ""), 42);
    ...
    public void CopyLog_button_appears_when_lines_stream_in_after_an_empty_project_log()
    {
        ...
        var header = NewHeaderWithIcons();
        header.ShowProjectLog(ConsoleHeaderRow.For("OSYS.Server.Api", ProjectRowState.Started, inCycle: false, depIssues: null, namePrefix: ""), 0);
        Assert.Equal(Visibility.Collapsed, header.CopyLogButton.Visibility);
    ...
    {
        var header = NewHeaderWithIcons();
        header.ShowProjectLog(ConsoleHeaderRow.For("A", ProjectRowState.Started, inCycle: false, depIssues: null, namePrefix: ""), 5);
